...
class CompletionGenerator {
  static {
    __name(this, "CompletionGenerator");
  }
  commands = [
    "start",
    "agent",
  ...
  async generate(shell, install = false) {
    ...
    switch (detectedShell) {
      ...
      case "fish":
        ...
        break;
      default:
        console.error(import_chalk.default.red(`Unsupported shell: ${detectedShell}`));
        console.log(import_chalk.default.gray("Supported shells: bash, zsh, fish"));
        break;
    }
  }
  async detectShell() {
    ...
    if (shell.includes("fish"))
      return "fish";
    console.log(import_chalk.default.yellow("Could not detect shell, defaulting to bash"));
    return "bash";
  }
  async generateBashCompletion(install) {
  ...
  async generateZshCompletion(install) {
  ...
  async generateFishCompletion(install) {
...
complete -f -c claude-flow -n '__fish_claude_flow_using_command completion' -a 'bash zsh fish'`;
  ...
  async installBashCompletion(script) {
    const possiblePaths = [
    ...
    for (const path of possiblePaths) {
      try {
        ...
        await import_node_fs.promises.writeFile(path, script);
        console.log(import_chalk.default.green("\u2713 Bash completion installed"));
        console.log(`${import_chalk.default.white("Location:")} ${path}`);
        console.log(import_chalk.default.gray("Restart your shell or run: source ~/.bashrc"));
        return;
      } catch (error) {
        continue;
      }
    }
    console.error(import_chalk.default.red("Failed to install bash completion"));
    console.log(
      import_chalk.default.gray("You can manually save the completion script to a bash completion directory")
    );
  }
  async installZshCompletion(script) {
    const possiblePaths = [
    ...
    for (const path of possiblePaths) {
      try {
        ...
        await Deno.mkdir(dir, { recursive: true });
        await import_node_fs.promises.writeFile(path, script);
        console.log(import_chalk.default.green("\u2713 Zsh completion installed"));
        console.log(`${import_chalk.default.white("Location:")} ${path}`);
        console.log(import_chalk.default.gray("Restart your shell or run: autoload -U compinit && compinit"));
        return;
      } catch (error) {
        continue;
      }
    }
    console.error(import_chalk.default.red("Failed to install zsh completion"));
    console.log(
      import_chalk.default.gray("You can manually save the completion script to your zsh completion directory")
    );
  }
  async installFishCompletion(script) {
    const possiblePaths = [
    ...
    for (const path of possiblePaths) {
      try {
        ...
        await Deno.mkdir(dir, { recursive: true });
        await import_node_fs.promises.writeFile(path, script);
        console.log(import_chalk.default.green("\u2713 Fish completion installed"));
        console.log(`${import_chalk.default.white("Location:")} ${path}`);
        console.log(import_chalk.default.gray("Completions will be available in new fish sessions"));
        return;
      } catch (error) {
        continue;
      }
    }
    console.error(import_chalk.default.red("Failed to install fish completion"));
    console.log(
      import_chalk.default.gray("You can manually save the completion script to your fish completion directory")
    );
  }
}
