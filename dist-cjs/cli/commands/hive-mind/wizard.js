...
async function spawnAgentInteractive(hiveMind) {
  ...
  const ora1 = require("ora");
  const spinner = ora1(`Spawning ${answers.count} ${answers.type} agent(s)...`).start();
  try {
    const agents = [];
...
async function submitTaskWizard() {
  ...
  const spinner = ora2("Submitting task...").start();
  try {
...
async function viewStatusWizard() {
  ...
  const statusCmd = statusModule.statusCommand;
  const args = ["status"];
  switch (view) {
    case "agents":
      args.push("--detailed");
...
async function rebalanceAgentsInteractive(hiveMind) {
  const ora3 = require("ora");
  const spinner = ora3("Rebalancing agents...").start();
  try {
