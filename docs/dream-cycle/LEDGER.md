...
**No-run gap (2026-09-13, 2026-09-14):** no branch/PR/issue exists for either date (verified via `git ls-remote --heads origin "dream/*"` and `search_pull_requests`, 2026-09-18). Tonight's automation SCAN confirms no in-repo cron/workflow drives this trigger, so the gap is genuinely unexplained from repo config, not a local misconfiguration — same open question as the 2026-08-20..23 gap.
| 2026-09-15 | performance | wire MoE gate's load-balance loss into the actual weight update (title-derived backfill; full PR body not re-verified tonight) | #3329 | #3330 | yes | ACCEPT-scoped | see issue #3329 / PR #3330 | n/a (backfill) | unmerged draft as of 2026-09-18, 3d old |
| 2026-09-16 | security | gate `hive-mind_spawn`/`propose`/`broadcast`/`shutdown`/`memory`/`optimize-memory` + `hive-mind_init`'s own credential issuance, round 2 (title-derived backfill; full PR body not re-verified tonight) | #3338 | #3339 | yes | ACCEPT | see issue #3338 / PR #3339 | n/a (backfill) | unmerged draft as of 2026-09-18, 2d old |
| 2026-09-17 | intelligence | wire `CLAUDE_FLOW_PRIOR_DECAY` env override for `ModelRouter`'s dormant discounted-Thompson-sampling decay primitive (#3049), recommended by 2026-09-12's own gist; real benchmark receipt (low-complexity bucket: recovery 26.5→21.9 rounds, t=7.0; med-complexity bucket: stationary Δ=−0.08pp, t=−3.01, accepted within pre-declared ±1pp tolerance) | #3349 | #3350 | yes | ACCEPT-with-caveats | 16/16 tests; full suite byte-identical 221 pre-existing failures both ways; independent adversarial critic CONFIRMED-WITH-CAVEATS (net-positive low-complexity, negligible-but-real med-complexity cost) | 91d8edc3... | unmerged draft as of 2026-09-18, 1d old |
| 2026-09-18 | memory | `AgentDBAdapter.store()` (default in-process write path) never checked for an existing entry under the same `(namespace,key)` before writing — a fresh random id every call left the prior occupant an orphan, unreachable via `getByKey()` but still live in `entries`/`namespaceIndex`/`tagIndex` and, for embedded entries, still a point in the HNSW index, so `search()`/`semanticSearch()` returned stale duplicates forever; fixed by evicting the prior occupant via a shared `evictEntry()` primitive before storing. Independently flagged by two prior nightly scans (09-12, 09-17) as a future-night candidate. **Post-review (ruvnet, same night):** two further public-path gaps found and fixed — `bulkInsert()` bypassed the same-key dedup entirely (fixed: pre-existing + intra-batch duplicate-key eviction), and `store()` deleted the prior occupant *before* validating the replacement embedding, risking data loss on a rejected write (fixed: validate/index first, evict on success) | #3351 | #3352 | yes | ACCEPT-scoped (post-review) | 9 discriminating tests total (4 original + 5 post-review, incl. a hand-built two-gate harness proving `setTimeout` can't reproduce the disclosed concurrency race); full suite 517→526 passed (1 pre-existing unrelated failure, unchanged); `tsc --noEmit` clean; independent adversarial critique CONFIRMED-WITH-CAVEATS pre-review, human review found 2 further blocking gaps post-review, both closed same night — concurrent-store race (not bulkInsert/atomicity) remains the one disclosed, still-open follow-up | d4d02062... | 09-08..09-12 (5 nights) MERGED in a 2026-09-14/15 batch; 09-15/09-16/09-17 still open/draft, none stale (<4d old); 09-13/09-14 no-run gap noted above |
