...
class AutoStrategy extends import_base.BaseStrategy {
  static {
    __name(this, "AutoStrategy");
  }
  mlHeuristics;
  decompositionCache;
  patternCache;
  ...
  constructor(config) {
    ...
    this.decompositionCache = /* @__PURE__ */ new Map();
  ...
  initializeMLHeuristics() {
    return {
      ...
      agentPerformanceHistory: /* @__PURE__ */ new Map(),
      complexityFactors: {
        integration: 1.5,
        system: 1.3,
        api: 1.2,
  ...
  async generateDevelopmentTasks(objective, complexity) {
    const tasks = [];
    const baseId = (0, import_helpers.generateId)("task");
    tasks.push(
      this.createTaskDefinition({
        ...
        priority: "high",
        estimatedDuration: Math.max(5 * 60 * 1e3, complexity * 3 * 60 * 1e3),
    ...
    tasks.push(
      this.createTaskDefinition({
        ...
        priority: "high",
        estimatedDuration: Math.max(8 * 60 * 1e3, complexity * 4 * 60 * 1e3),
    ...
    tasks.push(
      this.createTaskDefinition({
        ...
        priority: "medium",
        estimatedDuration: Math.max(5 * 60 * 1e3, complexity * 2 * 60 * 1e3),
    ...
    if (canParallelize && complexity >= 3) {
      ...
      components.forEach((component, index) => {
        tasks.push(
          this.createTaskDefinition({
            ...
            priority: "high",
            estimatedDuration: Math.max(10 * 60 * 1e3, complexity * 5 * 60 * 1e3),
      ...
      tasks.push(
        this.createTaskDefinition({
          ...
          priority: "high",
          estimatedDuration: Math.max(15 * 60 * 1e3, complexity * 8 * 60 * 1e3),
          capabilities: ["code-generation", "file-system"],
          dependencies: [`${baseId}-analysis`]
        })
      );
  ...
  async generateAnalysisTasks(objective, complexity) {
    ...
    tasks.push(
      this.createTaskDefinition({
        ...
        priority: "medium",
        estimatedDuration: Math.max(6 * 60 * 1e3, complexity * 3 * 60 * 1e3),
  ...
  determineOptimalTaskStructure(patterns, taskTypes, complexity) {
    return {
      ...
      analysisDuration: Math.max(5 * 60 * 1e3, complexity * 3 * 60 * 1e3),
      testingDuration: Math.max(5 * 60 * 1e3, complexity * 4 * 60 * 1e3)
    };
  }
  createOptimalImplementationTasks(objective, structure, baseId) {
    return [
      this.createTaskDefinition({
        ...
        priority: "high",
        estimatedDuration: Math.max(15 * 60 * 1e3, structure.complexity * 8 * 60 * 1e3),
        capabilities: ["code-generation", "file-system"],
  ...
  selectOptimalStrategy(objective, complexity) {
    ...
    if (objective.description.toLowerCase().includes("analyze"))
      return "analysis";
    if (objective.description.toLowerCase().includes("test"))
      return "testing";
    return "auto";
  }
  ...
  calculateCapabilityMatch(agent, task) {
    ...
    return requiredCaps.length > 0 ? matches / requiredCaps.length : 1;
  }
  ...
  applyMLHeuristics(agent, task) {
    ...
    const weight = this.mlHeuristics.taskTypeWeights[taskType] || 1;
    let bonus = 0;
    if (agent.type === "coder" && taskType === "development")
      bonus = 0.2;
    if (agent.type === "tester" && taskType === "testing")
      bonus = 0.2;
    if (agent.type === "analyst" && taskType === "analysis")
      bonus = 0.2;
    return Math.min(weight + bonus, 1);
  }
  ...
  async createPredictiveSchedule(tasks, agents) {
    ...
    for (const task of tasks) {
      const duration = task.constraints.timeoutAfter || 3e5;
      timeline.push({
        startTime: currentTime,
        endTime: currentTime + duration,
        tasks: [task.id.id],
        agents: [],
        // To be filled by allocation
