...
export class GeminiCliMCPWrapper {
  ...
  private async handleToolCall(toolName: string, args: any): Promise<CallToolResult> {
    try {
      ...
      // Pass through to Gemini CLI MCP
      return this.forwardToGeminiCli(toolName, args);
    } catch (error) {
      return {
        content: [{
          type: 'text',
  ...
  private async forwardToGeminiCli(toolName: string, args: any): Promise<CallToolResult> {
  ...
  async run() {
    ...
    // Log startup message
    console.error('🚀 Gemini-Flow MCP Server (Wrapper Mode)');
    console.error('📦 Using Gemini CLI MCP pass-through with SPARC prompt injection');
    console.error('🔧 All SPARC tools available with enhanced AI capabilities');
    console.error('ℹ️  To use legacy mode, set GEMINI_FLOW_LEGACY_MCP=true');
    console.error('');
