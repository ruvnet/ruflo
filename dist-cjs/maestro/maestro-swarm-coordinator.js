...
var maestro_swarm_coordinator_exports = {};
__export(maestro_swarm_coordinator_exports, {
  MaestroSwarmCoordinator: () => MaestroSwarmCoordinator
});
module.exports = __toCommonJS(maestro_swarm_coordinator_exports);
var import_events = require("events");
var import_path = require("path");
var import_promises = require("fs/promises");
var import_HiveMind = require("../hive-mind/core/HiveMind.js");
var import_errors = require("../utils/errors.js");
class MaestroSwarmCoordinator extends import_events.EventEmitter {
  constructor(config, eventBus, logger) {
    super();
    this.config = config;
    this.eventBus = eventBus;
    this.logger = logger;
    this.specsDirectory = config.specsDirectory || (0, import_path.join)(process.cwd(), ".claude", "claude-flow", "maestro", "specs");
    this.steeringDirectory = config.steeringDirectory || (0, import_path.join)(process.cwd(), ".claude", "claude-flow", "maestro", "steering");
    this.setupEventHandlers();
    this.logger.info("MaestroSwarmCoordinator initialized with native hive mind");
  }
  static {
    __name(this, "MaestroSwarmCoordinator");
  }
  hiveMind;
  maestroState = /* @__PURE__ */ new Map();
  specsDirectory;
  steeringDirectory;
  /**
  ...
  async initialize() {
    try {
      ...
      this.hiveMind = new import_HiveMind.HiveMind(hiveMindConfig);
  ...
  async createSpec(featureName, initialRequest) {
    const featurePath = (0, import_path.join)(this.specsDirectory, featureName);
    await (0, import_promises.mkdir)(featurePath, { recursive: true });
    const workflowState = {
      featureName,
    ...
    const requirementsTask = {
      ...
      metadata: {
        ...
        initialRequest,
        outputFile: (0, import_path.join)(featurePath, "requirements.md")
      }
    };
    const task = await this.hiveMind.submitTask(requirementsTask);
    await this.waitForTaskCompletion(task.id, 12e4);
  ...
  async generateDesign(featureName) {
    ...
    if (!state) {
      throw new import_errors.SystemError(`No workflow state found for '${featureName}'`);
    }
    const featurePath = (0, import_path.join)(this.specsDirectory, featureName);
    const requirementsPath = (0, import_path.join)(featurePath, "requirements.md");
    const requirementsContent = await (0, import_promises.readFile)(requirementsPath, "utf8");
    const designTask = {
      ...
      metadata: {
        ...
        outputFile: (0, import_path.join)(featurePath, "design.md")
      }
    };
    const task = await this.hiveMind.submitTask(designTask);
    await this.waitForTaskCompletion(task.id, 3e5);
  ...
  async generateTasks(featureName) {
    ...
    const featurePath = (0, import_path.join)(this.specsDirectory, featureName);
    const designPath = (0, import_path.join)(featurePath, "design.md");
    const designContent = await (0, import_promises.readFile)(designPath, "utf8");
    const taskPlanningTask = {
      ...
      metadata: {
        ...
        designContent,
        outputFile: (0, import_path.join)(featurePath, "tasks.md")
      }
    };
    const task = await this.hiveMind.submitTask(taskPlanningTask);
    await this.waitForTaskCompletion(task.id, 18e4);
  ...
  async implementTask(featureName, taskId) {
    ...
    const featurePath = (0, import_path.join)(this.specsDirectory, featureName);
    const tasksPath = (0, import_path.join)(featurePath, "tasks.md");
    const tasksContent = await (0, import_promises.readFile)(tasksPath, "utf8");
    const taskLines = tasksContent.split("\n").filter((line) => line.startsWith("- [ ]") || line.startsWith("- [x]"));
    if (taskId < 1 || taskId > taskLines.length) {
      throw new import_errors.SystemError(`Invalid task ID ${taskId} for feature '${featureName}'`);
    }
    ...
    await this.waitForTaskCompletion(task.id, 6e5);
    const updatedTasksContent = tasksContent.replace(
      taskLines[taskId - 1],
      taskLines[taskId - 1].replace("- [ ]", "- [x]")
    );
    await (0, import_promises.writeFile)(tasksPath, updatedTasksContent, "utf8");
  ...
  async reviewTasks(featureName) {
    ...
    const reviewTask = {
      ...
      priority: "high",
      strategy: "sequential",
      // Sequential validation for consistency
  ...
  async approvePhase(featureName) {
    ...
    if (this.config.enableConsensusValidation) {
      const consensusProposal = {
        ...
        deadline: new Date(Date.now() + 3e5),
        // 5 minutes
      ...
      const consensusResult = await this.waitForConsensusResult(proposalId, 3e5);
      if (!consensusResult.achieved) {
        throw new import_errors.SystemError(`Phase approval consensus failed: ${consensusResult.reason}`);
      }
    }
  ...
  async createSteeringDocument(domain, content) {
    if (!this.config.enableSteeringIntegration) {
      throw new import_errors.SystemError("Steering integration is disabled");
    }
