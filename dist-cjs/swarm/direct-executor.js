...
var direct_executor_exports = {};
__export(direct_executor_exports, {
  DirectTaskExecutor: () => DirectTaskExecutor
});
module.exports = __toCommonJS(direct_executor_exports);
const import_meta = {};
...
class DirectTaskExecutor {
  static {
    __name(this, "DirectTaskExecutor");
  }
  logger;
  timeout;
  constructor(config = {}) {
  ...
  async executeTaskByType(task, agent, targetDir) {
    ...
    switch (agent.type) {
      ...
      case "coder":
        if (isRestAPI)
          return this.createRestAPI(targetDir, task);
        if (isTodo)
          return this.createTodoApp(targetDir, task);
        if (isChat)
          return this.createChatApp(targetDir, task);
        if (isAuth)
          return this.createAuthService(targetDir, task);
        if (isHelloWorld)
          return this.createHelloWorld(targetDir, task);
        if (isCalculator)
          return this.createCalculator(targetDir, task);
...
//# sourceMappingURL=direct-executor.js.map
