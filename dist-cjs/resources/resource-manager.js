...
class ResourceManager extends import_node_events.EventEmitter {
  static {
  ...
  async registerResource(type, name, capacity, metadata = {}) {
    const resourceId = (0, import_helpers.generateId)("resource");
    const resource = {
      id: resourceId,
      type,
      name,
  ...
  async requestResources(agentId, requirements, options = {}) {
    const reservationId = (0, import_helpers.generateId)("reservation");
    const now = /* @__PURE__ */ new Date();
    const reservation = {
      id: reservationId,
      resourceId: "",
      // Will be set when resource is found
      agentId,
  ...
  async activateReservation(reservationId) {
    ...
    const allocationId = (0, import_helpers.generateId)("allocation");
    const allocation = {
      id: allocationId,
      reservationId,
      resourceId: resource.id,
    ...
    reservation.activatedAt = /* @__PURE__ */ new Date();
  ...
  async releaseResources(allocationId, reason = "completed") {
    ...
    allocation.endTime = /* @__PURE__ */ new Date();
    ...
    if (reservation) {
      reservation.releasedAt = /* @__PURE__ */ new Date();
    }
  ...
  async createResourcePool(name, type, resourceIds, strategy = "least-loaded") {
    ...
    const pool = {
      ...
      scaling: {
        ...
        cooldownPeriod: 3e5,
        metrics: [
          { name: "utilization", weight: 1, threshold: 0.8, aggregation: "avg" },
  ...
  calculateResourceScore(resource, requirements, priority) {
    ...
    const priorityWeight = this.config.priorityWeights[priority] || 1;
  ...
  calculateEfficiency(allocation) {
    ...
    return factors > 0 ? efficiencySum / factors : 1;
  }
  ...
  async updateResourceStatistics(resource) {
    ...
    if (history.length > 0) {
      ...
      resource.metadata.performance.cpuScore = Math.max(0.1, 1 - avgCpu / 100);
    }
    resource.metadata.lastUpdated = /* @__PURE__ */ new Date();
  }
  ...
  async checkQoSViolations() {
    for (const allocation of this.allocations.values()) {
      ...
      const pools = Array.from(this.pools.values()).filter(
        (p) => p.resources.includes(resource.id)
      );
  ...
  getManagerStatistics() {
    ...
    const avgEfficiency = activeAllocations.length > 0 ? activeAllocations.reduce((sum, a) => sum + a.efficiency, 0) / activeAllocations.length : 1;
    return {
      resources: this.resources.size,
      pools: this.pools.size,
...
class ResourceOptimizer {
  ...
  async predictUsage(resource, history) {
    ...
    for (let i = 1; i <= 24; i++) {
      const futureTime = new Date(Date.now() + i * 36e5);
      predictions.push({
...
class ResourceManagerMetrics {
  static {
    __name(this, "ResourceManagerMetrics");
  }
