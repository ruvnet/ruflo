...
1. **Requirements Analysis** (10 mins)
   ...
   - Query existing project context: \`npx gemini-flow memory query ${memoryNamespace}\`
   ...
   - Store findings: \`npx gemini-flow memory store ${memoryNamespace}_requirements "Core components: X, Y, Z. External deps: API-A, Service-B. Security: OAuth2, RLS policies needed."\`
...
2. **System Architecture Design** (20 mins)
   ...
   - Store architecture: \`npx gemini-flow memory store ${memoryNamespace}_architecture "Microservices: auth-service, user-service, data-processor. APIs: REST for external, gRPC for internal. State: Event-sourced with CQRS."\`
...
3. **Technical Specifications** (15 mins)
   ...
   - Store specs: \`npx gemini-flow memory store ${memoryNamespace}_tech_specs "Auth: JWT with refresh tokens. DB: PostgreSQL with read replicas. Cache: Redis. Config: Environment-based with secrets manager."\`
...
4. **Modular Implementation Plan** (10 mins)
   ...
   - Store plan: \`npx gemini-flow memory store ${memoryNamespace}_implementation_plan "Phase 1: Core auth (tdd mode). Phase 2: User management (code mode). Phase 3: Integration (integration mode)."\`
...
- \`npx gemini-flow sparc run spec-pseudocode "Create detailed pseudocode for ${taskDescription}" --non-interactive\`
- \`npx gemini-flow sparc run security-review "Review architecture for vulnerabilities" --non-interactive\`

### Background Swarm Mode (for complex implementation):
- \`npx gemini-flow swarm "Implement complete ${taskDescription} based on architecture" --strategy development --background --monitor\`
- \`npx gemini-flow swarm "Create comprehensive test suite for ${taskDescription}" --strategy testing --background --parallel\`
- \`npx gemini-flow swarm "Generate complete documentation for ${taskDescription}" --strategy research --background\`
...
npx gemini-flow swarm "Research and analyze all architectural patterns for ${taskDescription}" \\
...
npx gemini-flow swarm "Create architectural proof of concepts and prototypes for ${taskDescription}" \\
...
npx gemini-flow swarm "Generate complete technical specifications and API documentation for ${taskDescription}" \\
...
npx gemini-flow sparc run architect "Initial system design for ${taskDescription}" --non-interactive
npx gemini-flow swarm "Refine architecture based on security and performance analysis" \\
...
npx gemini-flow sparc run architect "Core system design" --non-interactive

# Phase 2: Parallel background research (Swarm)
npx gemini-flow swarm "Deep dive research into scalability patterns" --strategy research --background &
npx gemini-flow swarm "Security analysis and threat modeling" --strategy analysis --background &
npx gemini-flow swarm "Performance benchmarking and optimization research" --strategy optimization --background &

# Phase 3: Implementation swarm (Background)
npx gemini-flow swarm "Full implementation of ${taskDescription} architecture" \\
