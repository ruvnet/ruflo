...
var Queen_exports = {};
__export(Queen_exports, {
  Queen: () => Queen
});
module.exports = __toCommonJS(Queen_exports);
...
class Queen extends import_events.EventEmitter {
  static {
    __name(this, "Queen");
  }
  id;
  ...
  constructor(config) {
    ...
    this.taskQueue = /* @__PURE__ */ new Map();
  ...
  async scoreAgentForTask(agent, task, requiredCapabilities) {
    let score = 0;
    const capabilityMatches = requiredCapabilities.filter(
  ...
  async initiateConsensus(task, decision) {
    const proposal = {
      ...
      deadline: new Date(Date.now() + 5 * 60 * 1e3)
      // 5 minutes
    };
  ...
  isTaskStalled(task) {
    const stalledThreshold = 10 * 60 * 1e3;
