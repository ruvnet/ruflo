...
__export(swarm_tools_exports, {
  createSwarmTools: () => createSwarmTools,
  dispatchAgentTool: () => dispatchAgentTool,
  handleDispatchAgent: () => handleDispatchAgent,
  handleSwarmStatus: () => handleSwarmStatus,
  memoryRetrieveTool: () => memoryRetrieveTool,
  memoryStoreTool: () => memoryStoreTool,
  swarmStatusTool: () => swarmStatusTool,
  swarmTools: () => swarmTools
});
module.exports = __toCommonJS(swarm_tools_exports);
function createSwarmTools(logger) {
  return [
    // === LEGACY SWARM TOOLS ===
    {
      name: "dispatch_agent",
    ...
    },
    {
      ...
      handler: async (input, context) => {
        ...
        const runtime = Math.floor((Date.now() - mockState.startTime) / 1e3);
        return {
...
__name(createSwarmTools, "createSwarmTools");
...
async function handleDispatchAgent(args) {
...
__name(handleDispatchAgent, "handleDispatchAgent");
async function handleSwarmStatus(args) {
  const swarmId = process.env["CLAUDE_SWARM_ID"] || "default-swarm";
  const mockState = {
    swarmId,
    objective: "Legacy swarm status",
...
__name(handleSwarmStatus, "handleSwarmStatus");
const swarmTools = [dispatchAgentTool, memoryStoreTool, memoryRetrieveTool, swarmStatusTool];
