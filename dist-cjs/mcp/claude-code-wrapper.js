...
var claude_code_wrapper_exports = {};
__export(claude_code_wrapper_exports, {
  ClaudeCodeMCPWrapper: () => ClaudeCodeMCPWrapper
});
module.exports = __toCommonJS(claude_code_wrapper_exports);
const import_meta = {};
...
var import_sparc_modes = require("./sparc-modes.js");
function generateId() {
...
class ClaudeCodeMCPWrapper {
  static {
    __name(this, "ClaudeCodeMCPWrapper");
  }
  server;
  sparcModes = /* @__PURE__ */ new Map();
  swarmExecutions = /* @__PURE__ */ new Map();
  claudeCodeMCP;
  // Reference to Claude Code MCP client
  constructor() {
  ...
  async loadSparcModes() {
    try {
      const modes = await (0, import_sparc_modes.loadSparcModes)();
      modes.forEach((mode) => {
  ...
  setupHandlers() {
    ...
    this.server.setRequestHandler(
      ...
      async (request) => this.handleToolCall(request.params.name, request.params.arguments || {})
    );
  }
  async getTools() {
    const tools = [];
  ...
  async handleSparcTool(toolName, args) {
    ...
    try {
      const executeSparcMode2 = /* @__PURE__ */ __name((mode2, task, tools, context) => {
        throw new Error("SPARC mode execution not yet implemented in wrapper");
      }, "executeSparcMode");
      const result = await executeSparcMode2(
        mode,
        args.task,
        sparcMode.tools || [],
        args.context || {}
...
`);
  ...
  getSparcMethodology(mode, task, context) {
    return `
# \u{1F3AF} SPARC METHODOLOGY EXECUTION FRAMEWORK
...
## \u{1F680} Execution Configuration
...
## \u{1F4CB} Must Block (Non-negotiable)
...
## \u{1F3AF} IMMEDIATE ACTION REQUIRED
...
${list}`
  ...
  async run() {
    ...
    console.error("\u{1F680} Claude-Flow MCP Server (Wrapper Mode)");
    console.error("\u{1F4E6} Using Claude Code MCP pass-through with SPARC prompt injection");
    console.error("\u{1F527} All SPARC tools available with enhanced AI capabilities");
