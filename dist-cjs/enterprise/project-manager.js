...
module.exports = __toCommonJS(project_manager_exports);
var import_events = require("events");
var import_promises = require("fs/promises");
var import_path = require("path");
var import_logger = require("../core/logger.js");
var import_config = require("../core/config.js");
class ProjectManager extends import_events.EventEmitter {
  static {
    __name(this, "ProjectManager");
  }
  ...
  async initialize() {
    try {
      await (0, import_promises.mkdir)(this.projectsPath, { recursive: true });
  ...
  async createProject(projectData) {
    const project = {
      ...
      timeline: {
        plannedStart: projectData.timeline?.plannedStart || /* @__PURE__ */ new Date(),
        plannedEnd: projectData.timeline?.plannedEnd || new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3),
        // 30 days
  ...
  async deleteProject(projectId, userId = "system") {
    ...
    const archivePath = (0, import_path.join)(this.projectsPath, "archived");
    await (0, import_promises.mkdir)(archivePath, { recursive: true });
    await (0, import_promises.writeFile)((0, import_path.join)(archivePath, `${projectId}.json`), JSON.stringify(project, null, 2));
  ...
  async getProjectMetrics(projectId) {
    const projects = projectId ? [this.projects.get(projectId)].filter(Boolean) : Array.from(this.projects.values());
    ...
    const averageProjectDuration = completedProjectsWithDuration.length > 0 ? completedProjectsWithDuration.reduce((sum, p) => {
      ...
      return sum + duration / (1e3 * 60 * 60 * 24);
    ...
    return {
      ...
      qualityScore,
      riskScore: 0,
      // Calculate based on risk assessment
      teamProductivity: 0,
      // Calculate based on velocity metrics
  ...
  async loadProjects() {
    try {
      const files = await (0, import_promises.readdir)(this.projectsPath);
      const projectFiles = files.filter((f) => f.endsWith(".json") && !f.startsWith("."));
      for (const file of projectFiles) {
        try {
          const content = await (0, import_promises.readFile)((0, import_path.join)(this.projectsPath, file), "utf-8");
  ...
  async saveProject(project) {
    const filePath = (0, import_path.join)(this.projectsPath, `${project.id}.json`);
    await (0, import_promises.writeFile)(filePath, JSON.stringify(project, null, 2));
  }
  ...
  generateRiskMitigation(risks) {
    ...
    const highPriorityRisks = openRisks.filter(
      (r) => r.probability === "high" && r.impact === "high" || r.probability === "high" && r.impact === "medium" || r.probability === "medium" && r.impact === "high"
    );
    return {
