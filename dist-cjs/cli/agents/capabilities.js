...
__export(capabilities_exports, {
  AgentCapabilitySystem: () => AgentCapabilitySystem
});
module.exports = __toCommonJS(capabilities_exports);
class AgentCapabilitySystem {
  static {
    __name(this, "AgentCapabilitySystem");
  }
  capabilityRegistry;
  agentCapabilities;
  ...
  evaluateAgentMatch(agent, requirements) {
    ...
    if (requirements.languages) {
      maxScore += 15;
      const languageMatch = requirements.languages.some(
        (lang) => capabilities.languages.includes(lang)
      );
      if (languageMatch) {
        score += 15;
      }
    ...
    if (requirements.frameworks) {
      maxScore += 15;
      const frameworkMatch = requirements.frameworks.some(
        (framework) => capabilities.frameworks.includes(framework)
      );
      if (frameworkMatch) {
        score += 15;
      }
    ...
    if (requirements.domains) {
      maxScore += 10;
      const domainMatch = requirements.domains.some(
        (domain) => capabilities.domains.includes(domain)
      );
      if (domainMatch) {
        score += 10;
      }
  ...
  agentHasCapability(capabilities, capability) {
    ...
    const arrayFields = ["languages", "frameworks", "domains", "tools"];
  ...
  checkSemanticCapabilityMatch(capabilities, capability) {
    ...
    for (const [concept, related] of Object.entries(semanticMappings)) {
      if (capability.includes(concept)) {
        return related.some(
          (item) => capabilities.languages.includes(item) || capabilities.frameworks.includes(item) || capabilities.tools.includes(item)
        );
      }
    }
    return false;
  ...
  calculateAgentComplexityLevel(capabilities) {
    ...
    if (capabilities.terminalAccess)
      level += 0.5;
    if (capabilities.languages.length > 3)
      level += 0.5;
    if (capabilities.frameworks.length > 3)
      level += 0.5;
    if (capabilities.domains.length > 5)
      level += 0.5;
  ...
  calculateConfidence(matched, missing, requirements) {
    ...
    const criticalMissing = missing.filter(
      (cap) => requirements.requiredCapabilities.includes(cap)
    ).length;
