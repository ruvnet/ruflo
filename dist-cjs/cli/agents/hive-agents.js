...
var hive_agents_exports = {};
__export(hive_agents_exports, {
  ArchitectAgent: () => ArchitectAgent,
  GuardianAgent: () => GuardianAgent,
  HiveAgentFactory: () => HiveAgentFactory,
  QueenAgent: () => QueenAgent,
  ScoutAgent: () => ScoutAgent,
  WorkerAgent: () => WorkerAgent
});
module.exports = __toCommonJS(hive_agents_exports);
var import_base_agent = require("./base-agent.js");
class QueenAgent extends import_base_agent.BaseAgent {
  static {
    __name(this, "QueenAgent");
  }
...
class WorkerAgent extends import_base_agent.BaseAgent {
  static {
    __name(this, "WorkerAgent");
  }
  specialization;
...
- Solution-oriented`;
  ...
  async estimateEffort(task) {
    ...
    const specializationBonus = task.type === this.specialization ? 0.8 : 1;
    return Math.round(baseEffort * specializationBonus);
  }
}
class ScoutAgent extends import_base_agent.BaseAgent {
  static {
    __name(this, "ScoutAgent");
  }
  constructor(id, config, environment, logger, eventBus, memory) {
  ...
  getDefaultConfig() {
    return {
      ...
      maxTasksPerHour: 25,
      maxConcurrentTasks: 4,
      timeoutThreshold: 45e3,
      reportingInterval: 8e3,
...
class GuardianAgent extends import_base_agent.BaseAgent {
  static {
    __name(this, "GuardianAgent");
  }
  constructor(id, config, environment, logger, eventBus, memory) {
  ...
  getDefaultConfig() {
    return {
      ...
      maxTasksPerHour: 10,
      maxConcurrentTasks: 2,
      timeoutThreshold: 9e4,
      reportingInterval: 15e3,
...
class ArchitectAgent extends import_base_agent.BaseAgent {
  static {
    __name(this, "ArchitectAgent");
  }
  constructor(id, config, environment, logger, eventBus, memory) {
  ...
  getDefaultConfig() {
    return {
      ...
      maxTasksPerHour: 8,
      maxConcurrentTasks: 2,
      timeoutThreshold: 12e4,
      reportingInterval: 2e4,
...
class HiveAgentFactory {
  ...
  static createBalancedSwarm(objective, maxAgents = 8, agentConfig, environment, logger, eventBus, memory) {
    ...
    const workerCount = Math.min(3, maxAgents - agents.length - 1);
