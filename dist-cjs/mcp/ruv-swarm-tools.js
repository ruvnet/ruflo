...
var ruv_swarm_tools_exports = {};
__export(ruv_swarm_tools_exports, {
  createRuvSwarmTools: () => createRuvSwarmTools,
  default: () => ruv_swarm_tools_default,
  getRuvSwarmCapabilities: () => getRuvSwarmCapabilities,
  initializeRuvSwarmIntegration: () => initializeRuvSwarmIntegration,
  isRuvSwarmAvailable: () => isRuvSwarmAvailable
});
module.exports = __toCommonJS(ruv_swarm_tools_exports);
...
async function executeRuvSwarmCommand(command, args = [], context, logger) {
  try {
    ...
    const result = await (0, import_helpers.execAsync)(fullCommand, { cwd: workDir });
    let data;
    try {
...
__name(executeRuvSwarmCommand, "executeRuvSwarmCommand");
function createRuvSwarmTools(logger) {
  return [
...
__name(createRuvSwarmTools, "createRuvSwarmTools");
async function isRuvSwarmAvailable(logger) {
  try {
    const result = await executeRuvSwarmCommand("--version", [], void 0, logger);
...
__name(isRuvSwarmAvailable, "isRuvSwarmAvailable");
async function getRuvSwarmCapabilities(logger) {
  try {
...
__name(getRuvSwarmCapabilities, "getRuvSwarmCapabilities");
async function initializeRuvSwarmIntegration(workingDirectory, logger) {
  const context = {
    workingDirectory,
