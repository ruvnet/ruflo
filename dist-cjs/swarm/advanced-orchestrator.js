...
var advanced_orchestrator_exports = {};
__export(advanced_orchestrator_exports, {
  AdvancedSwarmOrchestrator: () => AdvancedSwarmOrchestrator,
  default: () => advanced_orchestrator_default
});
module.exports = __toCommonJS(advanced_orchestrator_exports);
...
var import_advanced_scheduler = require("../coordination/advanced-scheduler.js");
var import_swarm_monitor = require("../coordination/swarm-monitor.js");
var import_manager = require("../memory/manager.js");
var import_executor = __toESM(require("./executor.js"), 1);
class AdvancedSwarmOrchestrator extends import_node_events.EventEmitter {
  static {
    __name(this, "AdvancedSwarmOrchestrator");
  }
  logger;
  config;
  ...
  constructor(config = {}) {
    super();
    this.logger = new import_logger.Logger("AdvancedSwarmOrchestrator");
    this.config = this.createDefaultConfig(config);
    this.coordinator = new import_swarm_coordinator.SwarmCoordinator({
      maxAgents: this.config.maxAgents,
      maxConcurrentTasks: this.config.maxConcurrentTasks,
      taskTimeout: this.config.taskTimeoutMinutes * 60 * 1e3,
      enableMonitoring: this.config.realTimeMonitoring,
      coordinationStrategy: this.config.coordinationStrategy.name
    });
    this.memoryManager = new import_manager.MemoryManager(
      {
        backend: "sqlite",
  ...
  async createSwarm(objective, strategy = "auto", options = {}) {
    ...
    const context = {
      ...
      scheduler: new import_advanced_scheduler.AdvancedTaskScheduler({
      ...
      monitor: new import_swarm_monitor.SwarmMonitor({
        updateInterval: 1e3,
        enableAlerts: true,
        enableHistory: true,
        metricsRetention: 864e5
        // 24 hours
      }),
      memoryManager: this.memoryManager,
      taskExecutor: new import_executor.default({
        timeoutMs: this.config.taskTimeoutMinutes * 60 * 1e3,
  ...
  async startSwarm(swarmId) {
    ...
    try {
      ...
      context.objective.startedAt = /* @__PURE__ */ new Date();
  ...
  getOrchestratorMetrics() {
    ...
    return {
      ...
      system: {
        ...
        uptime: this.isRunning ? Date.now() - import_node_perf_hooks.performance.timeOrigin : 0,
  ...
  async performHealthCheck() {
    ...
    try {
      ...
      for (const [swarmId, context] of this.activeSwarms) {
        ...
        if (swarmAge > 36e5 && context.objective.status === "executing") {
  ...
  async decomposeObjective(objective) {
    const tasks = [];
    const baseTaskId = (0, import_helpers.generateId)("task");
  ...
  shouldFailSwarm(context) {
    ...
    if (context.objective.constraints.deadline && /* @__PURE__ */ new Date() > context.objective.constraints.deadline) {
      return true;
    }
    return false;
  }
  async completeSwarm(context) {
    ...
    context.objective.completedAt = /* @__PURE__ */ new Date();
    context.endTime = /* @__PURE__ */ new Date();
...
var advanced_orchestrator_default = AdvancedSwarmOrchestrator;
