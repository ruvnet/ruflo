...
| 2026-09-15 | performance | `MoERouter.route()` computed a Switch-Transformer-style load-balance auxiliary loss every call and returned it in `RoutingResult`, but `updateExpertWeights()` (the sole mutator of gate weights) never read that loss or its coefficient in its REINFORCE gradient math — the regularizer the module's own header claims had zero actual training effect; wired the analytic (softmax-Jacobian) gradient into the existing update path, using a per-`route()` fraction snapshot for consistency | #3329 | #3330 | yes | ACCEPT-scoped | 4 new discriminating tests, baseline-fails/candidate-passes via stash isolation (reproduced independently by critic); full `@claude-flow/neural` suite 132/132→136/136, 0 regressions; `tsc --noEmit` clean; independent adversarial critique CONFIRMED (scoping caveat: `updateExpertWeights()` has zero production callers today, though `route()` itself is live) | 5e719b96... | trailing 7 nights (#3086,#3139,#3152,#3160,#3232,#3243,#3266,#3291,#3302) all MERGED 2026-09-14 — full prior backlog cleared, reversing the earlier "0 of 14" signal |

**Note (2026-09-21):** #3302 confirmed **MERGED** 2026-09-15 via `pull_request_read` (authoritative) — `search_pull_requests`'s `merged` field is stale/unreliable (it reported `merged:false` with a populated `merged_at` for this same PR; do not trust it for merge-state checks, use `pull_request_read`/`get`). **New no-run gap, confirmed via `git ls-remote --heads origin "dream/*"`**: 2026-09-13 and 2026-09-14 have no branch, issue, or PR — same verification standard as the documented 08-20..08-23 gap, root cause still undiagnosed. 2026-09-15 through 2026-09-20 all ran (real branches confirmed): #3329/#3330 (perf, MoE load-balance), #3338/#3339 (security, hive-mind capability-token round 2), #3349/#3350 (intelligence, ModelRouter prior-decay env override), #3351/#3352 (memory, AgentDBAdapter upsert-orphan fix), #3356/#3357 (swarm, topology mirror-edge fix), #3377/#3378 (performance, UnifiedSwarmCoordinator busy-poll→event-driven, ~487x measured) — not transcribed row-by-row here per the established "ledger rows only reach `main` via PR merge" convention; all currently open/draft, none stale (<14d old as of tonight).

| 2026-09-21 | security | `validateEnv()`'s loader-hijack denylist blocked LD_PRELOAD/NODE_OPTIONS-class vars but not PATH itself or the equivalent interpreter/VCS search-path family (CWE-427) — flagged grade-A by 09-20's security scan; added PATH/PYTHONPATH/PERL5LIB/RUBYLIB/GIT_EXTERNAL_DIFF/GIT_SSH_COMMAND/BASH_ENV/IFS to the denylist. Adversarial critique found the lookup was case-sensitive (`path`/`Path` bypassed it on case-insensitive Windows) — fixed same session via `.toUpperCase()` case-folding, also hardening the original 9 names | #3384 | #3385 | yes | ACCEPT-scoped | 13 new discriminating tests (32/32 total); baseline-fails/candidate-passes via stash isolation, independently reproduced twice by the critic; full suite failed-file set byte-identical (36 pre-existing environmental failures); `tsc --noEmit` 14 pre-existing errors, unchanged; independent adversarial critique CONFIRMED-WITH-CAVEATS → caveat closed same session | 03142288... | 09-15..09-20 (6 nights) all open/draft, none stale; 08-24..09-11 backlog (10 PRs) still unmerged since the 09-14/09-15 batch that cleared 09-03/09-05/09-07/09-12 |
