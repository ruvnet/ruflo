...
Initialize a Gemini Flow swarm with specified topology and configuration.
...
```bash
npx gemini-flow swarm init --topology mesh --max-agents 5 --strategy balanced
```

### Hierarchical for development
```bash
npx gemini-flow swarm init --topology hierarchical --max-agents 10 --strategy parallel --auto-spawn
```

### GitHub-focused swarm
```bash
npx gemini-flow swarm init --topology star --github --memory
```
...
Once initialized, use MCP tools in Gemini Code:
```javascript
mcp__gemini-flow__swarm_init { topology: "hierarchical", maxAgents: 8 }
```
