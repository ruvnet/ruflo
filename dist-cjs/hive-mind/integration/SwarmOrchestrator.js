...
var SwarmOrchestrator_exports = {};
__export(SwarmOrchestrator_exports, {
  SwarmOrchestrator: () => SwarmOrchestrator
});
module.exports = __toCommonJS(SwarmOrchestrator_exports);
var import_events = require("events");
var import_DatabaseManager = require("../core/DatabaseManager.js");
var import_MCPToolWrapper = require("./MCPToolWrapper.js");
class SwarmOrchestrator extends import_events.EventEmitter {
  static {
    __name(this, "SwarmOrchestrator");
  }
  hiveMind;
  db;
  mcpWrapper;
  ...
  constructor(hiveMind) {
    ...
    this.executionPlans = /* @__PURE__ */ new Map();
  ...
  async executeParallel(task, plan, execution) {
    const parallelPhases = plan.phases.filter(
      (_, index) => plan.phaseAssignments[index].some((a) => a.canRunParallel)
    );
  ...
  async analyzeTaskComplexity(task) {
    ...
    return {
      ...
      estimatedDuration: analysis.data?.estimatedDuration || 36e5,
  ...
  getValidationCriteria(phase) {
    ...
    return criteria[phase] || [{ name: "completion", weight: 1 }];
  }
  /**
  ...
  async findSuitableAgent(requiredCapabilities) {
    ...
    const suitableAgents = agents.filter(
      (agent) => agent.status === "idle" && requiredCapabilities.every((cap) => agent.capabilities.includes(cap))
    );
  ...
  async waitForAgentCompletion(agent, taskId, timeout) {
    return new Promise((resolve, reject) => {
      ...
      const checkCompletion = /* @__PURE__ */ __name(async () => {
      ...
      const interval = setInterval(checkCompletion, 1e3);
    });
  }
  /**
  ...
  evaluateCriterion(result, criterion) {
    if (result.summary && result.summary.successRate !== void 0) {
