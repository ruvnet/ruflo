...
var message_bus_exports = {};
__export(message_bus_exports, {
  MessageBus: () => MessageBus
});
module.exports = __toCommonJS(message_bus_exports);
var import_node_events = require("node:events");
var import_helpers = require("../utils/helpers.js");
class MessageBus extends import_node_events.EventEmitter {
  static {
  ...
  deliveryReceipts = /* @__PURE__ */ new Map();
  acknowledgments = /* @__PURE__ */ new Map();
  ...
  constructor(config, logger, eventBus) {
    ...
    this.config = {
      ...
      // 1MB
      maxQueueSize: 1e4,
      messageRetention: 864e5,
      // 24 hours
      acknowledgmentTimeout: 3e4,
      retryAttempts: 3,
      backoffMultiplier: 2,
  ...
  async sendMessage(type, content, sender, receivers, options = {}) {
    const messageId = (0, import_helpers.generateId)("msg");
  ...
  async subscribeToTopic(topic, subscriber, options = {}) {
    const subscriptionId = (0, import_helpers.generateId)("sub");
    const subscription = {
      id: subscriptionId,
      topic,
      subscriber,
  ...
  async createChannel(name, type, config = {}) {
    const channelId = (0, import_helpers.generateId)("channel");
    const channel = {
      id: channelId,
      name,
      type,
  ...
  async createQueue(name, type, config = {}) {
    const queueId = (0, import_helpers.generateId)("queue");
    const queue = {
      id: queueId,
      name,
      type,
  ...
  async dequeueMessage(queueId, subscriberId) {
    ...
    subscriber.lastActivity = /* @__PURE__ */ new Date();
  ...
  validateMessage(message) {
    ...
    if (message.expiresAt && message.expiresAt <= /* @__PURE__ */ new Date()) {
  ...
  checkAllAcknowledgments(message) {
    ...
    const receivedAcks = message.receivers.filter(
      (receiver) => this.acknowledgments.has(`${message.id}:${receiver.id}`)
    ).length;
  ...
  updateChannelStatistics(channel) {
    channel.statistics.lastActivity = /* @__PURE__ */ new Date();
  }
  ...
  async sendToDeadLetterQueue(queueId, message, reason) {
    try {
      ...
      message.metadata.deadLetterTimestamp = /* @__PURE__ */ new Date();
...
class DeliveryManager extends import_node_events.EventEmitter {
  constructor(config, logger) {
    super();
    this.config = config;
    this.logger = logger;
...
class RetryManager extends import_node_events.EventEmitter {
  constructor(config, logger) {
    super();
    this.config = config;
    this.logger = logger;
...
class MessageBusMetrics {
  ...
  recordDeliverySuccess(message) {
    ...
    if (this.deliveryLatencies.length > 1e3) {
