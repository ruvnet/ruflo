...
class RealTimeMonitor extends import_node_events.EventEmitter {
  static {
    __name(this, "RealTimeMonitor");
  }
  logger;
  eventBus;
  memory;
  ...
  constructor(config, logger, eventBus, memory) {
    ...
    this.config = {
      ...
      alertThresholds: {
        ...
        responseTime: { warning: 5e3, critical: 1e4 },
  ...
  resolveAlert(alertId, reason) {
    ...
    alert.context.resolvedAt = /* @__PURE__ */ new Date();
  ...
  cleanupResolvedAlerts() {
    ...
    this.alertHistory = this.alertHistory.filter((alert) => alert.timestamp > cutoff).slice(-1e3);
  }
  async flushMetrics() {
    if (this.metricsBuffer.length > 0) {
      this.processMetricsBuffer();
