...
echo "🔨 Building Gemini Flow with workaround..."
...
(deno compile --allow-all --no-check --output=bin/gemini-flow.tmp src/cli/main.ts 2>&1 || true) | grep -v "Import assertions are deprecated"
...
if [ -f "bin/gemini-flow.tmp" ]; then
    ...
    if bin/gemini-flow.tmp --version &>/dev/null; then
        ...
        mv bin/gemini-flow.tmp bin/gemini-flow
...
if deno bundle src/cli/main.ts bin/gemini-flow.bundle.js &>/dev/null; then
    # Create a wrapper script
    cat > bin/gemini-flow.new << 'EOF'
#!/usr/bin/env -S deno run --allow-all --no-check
import "./gemini-flow.bundle.js";
EOF
    
    chmod +x bin/gemini-flow.new
    
    # Test the wrapper
    if bin/gemini-flow.new --version &>/dev/null; then
        ...
        mv bin/gemini-flow.new bin/gemini-flow
