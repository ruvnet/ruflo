...
__export(hook_exports, {
  ...
  hookSubcommands: () => hookSubcommands
});
...
function buildArgs(hookType, options) {
  const args = [hookType];
...
async function executeHook(hookType, options) {
  const args = buildArgs(hookType, options);
  logger.debug(`Executing hook: ruv-swarm hook ${args.join(" ")}`);
  const child = (0, import_child_process.spawn)("npx", ["ruv-swarm", "hook", ...args], {
    stdio: "inherit",
    shell: true
  });
...
__name(showHookHelp, "showHookHelp");
