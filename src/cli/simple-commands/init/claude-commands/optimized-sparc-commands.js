...
1. **Run directly with parallel processing**: \`./gemini-flow sparc run ${mode.slug} "your task" --parallel\`
2. **Batch operation mode**: \`./gemini-flow sparc batch ${mode.slug} "tasks-file.json" --concurrent\`
3. **Pipeline processing**: \`./gemini-flow sparc pipeline ${mode.slug} "your task" --stages\`
...
./gemini-flow sparc run ${mode.slug} "${getOptimizedExampleTask(mode.slug)}"

# Use with memory namespace and parallel processing
./gemini-flow sparc run ${mode.slug} "your task" --namespace ${mode.slug} --parallel

# Non-interactive mode with batchtools optimization
./gemini-flow sparc run ${mode.slug} "your task" --non-interactive --batch-optimize
\`\`\`

### Batchtools Operations
\`\`\`bash
# Parallel execution with multiple related tasks
./gemini-flow sparc parallel ${mode.slug} "task1,task2,task3" --concurrent

# Batch processing from configuration file
./gemini-flow sparc batch ${mode.slug} tasks-config.json --optimize

# Pipeline execution with staged processing
./gemini-flow sparc pipeline ${mode.slug} "complex-task" --stages parallel,validate,optimize
\`\`\`

### Performance Optimization
\`\`\`bash
# Monitor performance during execution
./gemini-flow sparc run ${mode.slug} "your task" --monitor --performance

# Use concurrent processing with resource limits
./gemini-flow sparc concurrent ${mode.slug} "your task" --max-parallel 5 --resource-limit 80%

# Batch execution with smart optimization
./gemini-flow sparc smart-batch ${mode.slug} "your task" --auto-optimize --adaptive
\`\`\`
...
./gemini-flow memory store "${mode.slug}_context" "important decisions" --namespace ${mode.slug}

# Query previous work
./gemini-flow memory query "${mode.slug}" --limit 5
\`\`\`

### Batchtools Memory Operations
\`\`\`bash
# Batch store multiple related contexts
./gemini-flow memory batch-store "${mode.slug}_contexts.json" --namespace ${mode.slug} --parallel

# Concurrent query across multiple namespaces
./gemini-flow memory parallel-query "${mode.slug}" --namespaces ${mode.slug},project,arch --concurrent

# Export mode-specific memory with compression
./gemini-flow memory export "${mode.slug}_backup.json" --namespace ${mode.slug} --compress --parallel
\`\`\`
...
./gemini-flow sparc concurrent ${mode.slug},architect,security-review "your project" --parallel

# Pipeline execution across multiple modes
./gemini-flow sparc pipeline ${mode.slug}->code->tdd "feature implementation" --optimize
\`\`\`

### Batch Workflow Integration
\`\`\`bash
# Execute complete workflow with batchtools optimization
./gemini-flow sparc workflow ${mode.slug}-workflow.json --batch-optimize --monitor
\`\`\`

For detailed ${mode.name} documentation and batchtools integration guides, see: 
- Mode Guide: https://github.com/ruvnet/gemini-flow/docs/sparc-${mode.slug}.md
- Batchtools Integration: https://github.com/ruvnet/gemini-flow/docs/batchtools-${mode.slug}.md
`;
}
...
export function createOptimizedMainSparcCommand(modes) {
  ...
  return `---
name: sparc
description: Execute SPARC methodology workflows with Gemini-Flow and batchtools optimization
---
...
\`\`\`bash
./gemini-flow sparc "build complete authentication system" --parallel --optimize
\`\`\`

### Run multiple modes concurrently:
\`\`\`bash
./gemini-flow sparc concurrent architect,code,tdd "your project" --parallel
\`\`\`

### Execute batch operations:
\`\`\`bash
./gemini-flow sparc batch "multiple-tasks.json" --optimize --monitor
\`\`\`

### Pipeline execution with staged processing:
\`\`\`bash
./gemini-flow sparc pipeline "complex-project" --stages spec,architect,code,tdd,integration
\`\`\`
...
./gemini-flow memory parallel-query "authentication" --namespaces spec,arch,impl --concurrent

# Export project memory with compression
./gemini-flow memory export sparc-project-backup.json --compress --parallel
\`\`\`
...
./gemini-flow swarm "Build e-commerce platform" --strategy development --monitor --review --parallel

# Background optimization swarm with concurrent processing
./gemini-flow swarm "Optimize system performance" --strategy optimization --background --concurrent

# Distributed research swarm with batch analysis
./gemini-flow swarm "Analyze market trends" --strategy research --distributed --ui --batch-analyze
\`\`\`
...
\`\`\`bash
./gemini-flow sparc run code "implement API" --non-interactive --parallel
./gemini-flow sparc batch tdd "user tests" --non-interactive --enable-permissions --concurrent
./gemini-flow sparc pipeline "full-stack-app" --non-interactive --optimize --stages parallel
\`\`\`
...
./gemini-flow sparc monitor --real-time --performance --all-phases

# Analyze batch operation efficiency
./gemini-flow sparc analyze --batchtools --optimization --detailed

# Performance comparison across modes
./gemini-flow sparc compare --modes architect,code,tdd --performance
\`\`\`

### Optimization Commands
\`\`\`bash
# Optimize SPARC configuration for your system
./gemini-flow sparc optimize --auto-tune --system-profile

# Performance benchmarking
./gemini-flow sparc benchmark --all-modes --detailed --export-results
\`\`\`
...
./gemini-flow sparc debug --resources --concurrent --verbose

# Analyze batch operation performance
./gemini-flow sparc analyze --performance --bottlenecks --optimization

# Monitor parallel processing efficiency
./gemini-flow sparc monitor --parallel --efficiency --real-time
\`\`\`
...
See \`/gemini-flow-help\` for all available commands and \`/batchtools\` for detailed parallel processing documentation.
