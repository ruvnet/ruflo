...
USAGE:
    ./gemini-sparc.sh [OPTIONS] [PROJECT_NAME] [README_PATH]
...
EXAMPLES:
    # Basic usage
    ./gemini-sparc.sh my-app docs/requirements.md
    
    # Backend API development with verbose output
    ./gemini-sparc.sh --mode api-only --verbose user-service api-spec.md
    
    # Quick prototype without research
    ./gemini-sparc.sh --skip-research --coverage 80 prototype-app readme.md
    
    # Dry run to see what would be executed
    ./gemini-sparc.sh --dry-run --verbose my-project requirements.md
...
main() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        echo "Gemini Flags: $(build_gemini_flags)"
        exit 0
    fi
...
execute_sparc_development() {
    ...
    local gemini_flags=$(build_gemini_flags)
