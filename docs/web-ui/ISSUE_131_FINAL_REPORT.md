...
class NeuralNetworksPanel extends EventEmitter {
    async executeTool(toolName, params) {
        const response = await this.ws.sendRequest('tools/call', {
            name: `mcp__gemini-flow__${toolName}`,
            arguments: params
        });
        return response;
    }
...
- **Accessibility**: Remote access to all Gemini Flow features
...
**Date**: 2025-07-06  
**Project**: Gemini Flow v2.0.0 Web UI Enhancement  
**Issue**: #131  
