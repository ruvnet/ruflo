...
var result_aggregator_exports = {};
__export(result_aggregator_exports, {
  SwarmResultAggregator: () => SwarmResultAggregator,
  default: () => result_aggregator_default
});
module.exports = __toCommonJS(result_aggregator_exports);
var import_node_events = require("node:events");
var import_node_perf_hooks = require("node:perf_hooks");
var import_logger = require("../core/logger.js");
var import_helpers = require("../utils/helpers.js");
class SwarmResultAggregator extends import_node_events.EventEmitter {
  static {
    __name(this, "SwarmResultAggregator");
  }
  logger;
  config;
  memoryManager;
  activeAggregations = /* @__PURE__ */ new Map();
  resultCache = /* @__PURE__ */ new Map();
  processingQueue;
  constructor(config = {}, memoryManager) {
    super();
    this.logger = new import_logger.Logger("SwarmResultAggregator");
  ...
  async startAggregation(context) {
    const aggregationId = (0, import_helpers.generateId)("aggregation");
  ...
  async createReport(result, format) {
    const reportId = (0, import_helpers.generateId)("report");
    ...
    const processingTime = import_node_perf_hooks.performance.now() - startTime;
    const report = {
      id: reportId,
  ...
  calculateThroughput(result) {
    return result.dataPoints / (result.processingTime / 1e3);
  }
  ...
  identifyImprovementAreas(metrics) {
    ...
    if (metrics.completeness < threshold)
      improvements.push("Coverage gaps identified");
    if (metrics.consistency < threshold)
      improvements.push("Output consistency issues");
    if (metrics.timeliness < threshold)
      improvements.push("Execution time optimization needed");
    if (metrics.reliability < threshold)
      improvements.push("Reliability concerns");
    return improvements;
  }
...
class AggregationSession {
  static {
    __name(this, "AggregationSession");
  }
  id;
  context;
  config;
  ...
  async finalize() {
    ...
    const processingStartTime = import_node_perf_hooks.performance.now();
    ...
    const processingTime = import_node_perf_hooks.performance.now() - processingStartTime;
    const result = {
      id: this.id,
      swarmId: this.context.swarmId.id,
      timestamp: this.startTime,
  ...
  async generateRecommendations() {
    return [
      {
        id: (0, import_helpers.generateId)("recommendation"),
...
var result_aggregator_default = SwarmResultAggregator;
