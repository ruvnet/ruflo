...
**Note (2026-09-17):** verified via GitHub MCP tools (not inferred) before tonight's own row: **2026-09-13 and 2026-09-14 are genuine no-run gaps** — no `dream/2026-09-13-*`/`dream/2026-09-14-*` branch, no matching issue/PR, same class as the earlier 2026-08-20..23 gap. **2026-09-15 ran** (`performance`, #3329 — MoE gate load-balance loss computed but never applied to weights) and **2026-09-16 ran** (`security`, #3338 — hive-mind consensus bypass); neither transcribed as its own row here (out of scope for tonight's `intelligence` slot to backfill in full — flagging their existence is enough to keep STEP 1's gap-detection honest). Trailing-14 PR merge rate: **14/14 MERGED** — a bulk human-review sweep on 2026-09-14/15 cleared the entire backlog from #3086 through #3302 in one pass. This is the opposite extreme from the "0 of last 14" signal earlier nights tracked, so tonight did **not** bias toward an artificially small candidate on that basis — but the bulk-sweep nature (many PRs merged together) is a weaker scrutiny signal than a steady one-by-one trickle, noted for future nights' calibration.

| 2026-09-17 | intelligence | `ModelRouter`'s discounted-Thompson-sampling `priorDecay` primitive (built/tested/benchmarked in #3049, 2026-08-17) shipped permanently inert — `DEFAULT_CONFIG.priorDecay` hardcoded to `1` (disabled), no env/config override unlike sibling `maxUncertainty`; added `envPriorDecay()` mirroring the existing `envMaxUncertainty()` pattern exactly, acting on 09-12's own gist recommendation for a future `intelligence` night | #3349 | #3350 | yes | ACCEPT-scoped | stash-isolated: 1/3 new tests fail reverted, 16/16 pass restored; full `@claude-flow/cli` suite + `tsc --noEmit` byte-identical baseline vs candidate outside the 3 new tests (221 failures / 463 errors both ways); benchmark re-run byte-identical to 2026-08-17 receipt — real non-stationary recovery win in 'low' bucket (t=7.0), small but statistically real stationary-accuracy cost in 'med' bucket (t=-3.01, 0.08pp, inside the original ±1pp tolerance); independent adversarial critic CONFIRMED-WITH-CAVEATS, disclosed not oversold | 91d8edc3... | 08-24..09-11 backlog (10 PRs) + 09-12 (#3302) all MERGED 2026-09-14/15 in a bulk sweep; 09-13/09-14 no-run gap, 09-15/09-16 ran but not transcribed (see note above) |
