...
class TaskExecutor extends import_node_events.EventEmitter {
  static {
  ...
  async executeTask(task, agent, options = {}) {
    const sessionId = (0, import_helpers.generateId)("execution");
  ...
  async executeClaudeTask(task, agent, claudeOptions = {}) {
    const sessionId = (0, import_helpers.generateId)("claude-execution");
    const context = await this.createExecutionContext(task, agent);
  ...
  async executeClaudeWithTimeout(sessionId, task, agent, context, options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (command.input && process2.stdin) {
          process2.stdin.write(command.input);
          process2.stdin.end();
        }
        if (options.detached) {
          process2.unref();
  ...
  buildClaudePrompt(task, agent) {
    ...
    sections.push(
      `- Maximum execution time: ${task.constraints.timeoutAfter || import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT}ms`
    );
    sections.push(
      `- Maximum retries: ${task.constraints.maxRetries || import_types.SWARM_CONSTANTS.MAX_RETRIES}`
    );
  ...
  async createExecutionContext(task, agent) {
    ...
    return {
      ...
      resources: {
        maxMemory: task.requirements.memoryRequired || import_types.SWARM_CONSTANTS.DEFAULT_MEMORY_LIMIT,
        maxCpuTime: task.requirements.maxDuration || import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT,
  ...
  mergeWithDefaults(config) {
    return {
      timeoutMs: import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT,
      retryAttempts: import_types.SWARM_CONSTANTS.MAX_RETRIES,
      killTimeout: 5e3,
      // 5 seconds
      resourceLimits: {
        maxMemory: import_types.SWARM_CONSTANTS.DEFAULT_MEMORY_LIMIT,
        maxCpuTime: import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT,
        maxDiskSpace: 1024 * 1024 * 1024,
        // 1GB
        maxNetworkConnections: 10,
        maxFileHandles: 100,
...
class ResourceMonitor extends import_node_events.EventEmitter {
  static {
    __name(this, "ResourceMonitor");
  }
...
class ProcessPool extends import_node_events.EventEmitter {
  static {
    __name(this, "ProcessPool");
  }
  config;
  totalExecutions = 0;
  totalDuration = 0;
...
var executor_default = TaskExecutor;
