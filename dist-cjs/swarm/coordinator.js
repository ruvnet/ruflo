...
var import_auto = require("./strategies/auto.js");
var import_paths = require("../utils/paths.js");
var import_json_output_aggregator = require("./json-output-aggregator.js");
class SwarmCoordinator extends import_events.EventEmitter {
  static {
  ...
  constructor(config = {}) {
    ...
    this.autoStrategy = new import_auto.AutoStrategy(config);
  ...
  async initialize() {
    ...
    try {
      ...
      this.emitSwarmEvent({
        id: (0, import_helpers.generateId)("event"),
  ...
  async createObjective(name, description, strategy = "auto", requirements = {}) {
    const objectiveId = (0, import_helpers.generateId)("objective");
    const objective = {
      id: objectiveId,
      name,
      description,
      ...
      requirements: {
        ...
        estimatedDuration: 60 * 60 * 1e3,
        // 1 hour default
        maxDuration: 4 * 60 * 60 * 1e3,
        // 4 hours default
  ...
  async executeObjective(objectiveId) {
    ...
    objective.startedAt = /* @__PURE__ */ new Date();
    try {
  ...
  async registerAgent(name, type, capabilities = {}) {
    const agentId = {
      id: (0, import_helpers.generateId)("agent"),
      swarmId: this.swarmId.id,
      type,
    ...
    const agentState = {
      ...
      capabilities: {
        ...
        maxMemoryUsage: import_types.SWARM_CONSTANTS.DEFAULT_MEMORY_LIMIT,
        maxExecutionTime: import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT,
        reliability: 0.8,
        speed: 1,
        quality: 0.8,
        ...capabilities
      ...
      config: {
        ...
        timeoutThreshold: import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT,
        reportingInterval: 3e4,
        heartbeatInterval: import_types.SWARM_CONSTANTS.DEFAULT_HEARTBEAT_INTERVAL,
  ...
  async createTask(type, name, description, instructions, options = {}) {
    ...
    const task = {
      ...
      constraints: {
        ...
        dependents: [],
        conflicts: [],
        maxRetries: import_types.SWARM_CONSTANTS.MAX_RETRIES,
        timeoutAfter: import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT,
        ...options.constraints
      },
      priority: "normal",
      input: options.input || {},
  ...
  async assignTask(taskId, agentId) {
    ...
    task.assignedAt = /* @__PURE__ */ new Date();
  ...
  async completeTask(taskId, result) {
    ...
    if (currentAttempt) {
      currentAttempt.completedAt = /* @__PURE__ */ new Date();
    ...
    agent.metrics.lastActivity = /* @__PURE__ */ new Date();
  ...
  async failTask(taskId, error) {
    ...
    const shouldRetry = task.error.retryable && task.attempts.length < (task.constraints.maxRetries || import_types.SWARM_CONSTANTS.MAX_RETRIES);
    if (shouldRetry) {
      ...
      const retryDelay = Math.pow(2, task.attempts.length) * 1e3;
      setTimeout(() => {
  ...
  generateSwarmId() {
    return {
      id: (0, import_helpers.generateId)("swarm"),
      timestamp: Date.now(),
      namespace: "default"
    };
  }
  mergeWithDefaults(config) {
    return {
      ...
      // 4 hours
      resourceLimits: {
        memory: import_types.SWARM_CONSTANTS.DEFAULT_MEMORY_LIMIT,
        cpu: import_types.SWARM_CONSTANTS.DEFAULT_CPU_LIMIT,
        disk: import_types.SWARM_CONSTANTS.DEFAULT_DISK_LIMIT
      },
      qualityThreshold: import_types.SWARM_CONSTANTS.DEFAULT_QUALITY_THRESHOLD,
      reviewRequired: true,
      ...
      performance: {
        maxConcurrency: 10,
        defaultTimeout: import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT,
        cacheEnabled: true,
        cacheSize: 100,
        cacheTtl: 36e5,
        // 1 hour
  ...
  initializeProgress() {
    return {
      ...
      failedTasks: 0,
      runningTasks: 0,
      estimatedCompletion: /* @__PURE__ */ new Date(),
      timeRemaining: 0,
      percentComplete: 0,
      averageQuality: 0,
      passedReviews: 0,
  ...
  async decomposeObjective(objective) {
    ...
    const targetPath = targetDir ? targetDir.startsWith("/") ? targetDir : `${(0, import_paths.getClaudeFlowRoot)()}/${targetDir}` : null;
...
Ensure your implementation is complete, well-structured, and follows best practices.`,
  ...
  createTaskForObjective(id, type, params) {
    ...
    return {
      ...
      constraints: {
        ...
        conflicts: [],
        maxRetries: import_types.SWARM_CONSTANTS.MAX_RETRIES,
        timeoutAfter: params.estimatedDuration || import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT
      },
  ...
  startTaskExecutionLoop(objective) {
    ...
    const executionInterval = setInterval(async () => {
      try {
        ...
        const pendingTasks = Array.from(this.tasks.values()).filter(
          (task) => task.context?.objectiveId === objective.id && task.status === "created" && this.taskDependenciesMet(task, completedTasks)
        );
        ...
        for (const task of runningTasks) {
          if (task.startedAt) {
            ...
            const timeout = task.constraints?.timeoutAfter || import_types.SWARM_CONSTANTS.DEFAULT_TASK_TIMEOUT;
            if (runtime > timeout) {
              this.logger.warn("Task timed out", {
                taskId: task.id.id,
                runtime: Math.round(runtime / 1e3),
                timeout: Math.round(timeout / 1e3)
              });
              task.status = "failed";
              task.completedAt = /* @__PURE__ */ new Date();
              task.error = {
        ...
        if (objective.progress.completedTasks + objective.progress.failedTasks === objective.progress.totalTasks) {
          ...
          objective.completedAt = /* @__PURE__ */ new Date();
          clearInterval(executionInterval);
    ...
    if (!this.executionIntervals) {
      this.executionIntervals = /* @__PURE__ */ new Map();
    }
  ...
  async executeTaskWithAgent(task, agent) {
    ...
    try {
      ...
      const executor = new ClaudeFlowExecutor({
        logger: this.logger,
        claudeFlowPath: (0, import_paths.getClaudeFlowBin)(),
        enableSparc: true,
...
`;
  ...
  extractTargetDirectory(task) {
    const patterns = [
      /in\s+([^\s]+\/?)$/i,
      // "in examples/dir" at end
      /(?:in|to|at)\s+([^\s]+\/[^\s]+)/i,
      // "in examples/gradio" anywhere
      /([^\s]+\/[^\s]+)$/,
      // "examples/gradio" at end
      /examples\/[^\s]+/i
      // specifically match examples/ paths
    ];
    let targetDir = null;
    ...
    if (targetDir) {
      ...
      if (!targetDir.startsWith("/")) {
        targetDir = `${(0, import_paths.getClaudeFlowRoot)()}/${targetDir}`;
      }
    }
    return targetDir;
  }
...
Please ensure all files are created in: ${targetDir}`;
  ...
  async simulateTaskExecution(task, agent, prompt) {
    ...
    if (targetDir) {
      targetDir = targetDir.replace(/\s+.*$/, "");
      workDir = targetDir.startsWith("/") ? targetDir : `${(0, import_paths.getClaudeFlowRoot)()}/${targetDir}`;
  ...
  async executeCodeGenerationTask(task, workDir, agent) {
    ...
    if (isGradio) {
      ...
      const projectDir2 = `${workDir}/${projectName}`;
      await Deno.mkdir(projectDir2, { recursive: true });
...
`;
      await import_node_fs.promises.writeFile(`${projectDir2}/server.js`, apiCode);
      const packageJson = {
        name: projectName,
        version: "1.0.0",
      ...
      await import_node_fs.promises.writeFile(`${projectDir2}/package.json`, JSON.stringify(packageJson, null, 2));
...
`;
      await import_node_fs.promises.writeFile(`${projectDir2}/README.md`, readme);
...
coverage/
`;
      await import_node_fs.promises.writeFile(`${projectDir2}/.gitignore`, gitignore);
      return {
        success: true,
        output: {
          message: "REST API created successfully",
          location: projectDir2,
          files: ["server.js", "package.json", "README.md", ".gitignore"]
        },
        artifacts: {
          mainFile: `${projectDir2}/server.js`,
          packageFile: `${projectDir2}/package.json`,
          readmeFile: `${projectDir2}/README.md`
        }
      };
    } else if (isHelloWorld) {
      const projectDir2 = `${workDir}/hello-world`;
      await Deno.mkdir(projectDir2, { recursive: true });
...
`;
      await import_node_fs.promises.writeFile(`${projectDir2}/index.js`, mainCode);
      const packageJson = {
        name: "hello-world",
        version: "1.0.0",
...
main();
`;
    await import_node_fs.promises.writeFile(`${projectDir}/main.js`, code);
    return {
      success: true,
      output: {
  ...
  async executeAnalysisTask(task, workDir, agent) {
    ...
    await import_node_fs.promises.writeFile(`${analysisDir}/analysis-report.json`, JSON.stringify(analysis, null, 2));
    return {
      success: true,
      output: analysis,
      artifacts: {
...
`;
    await import_node_fs.promises.writeFile(`${docsDir}/documentation.md`, documentation);
    return {
      success: true,
      output: {
...
`;
    await import_node_fs.promises.writeFile(`${testDir}/test.js`, testCode);
    return {
      success: true,
      output: {
  ...
  async executeGenericTask(task, workDir, agent) {
    ...
    await import_node_fs.promises.writeFile(`${outputDir}/result.json`, JSON.stringify(output, null, 2));
    return {
      success: true,
      output,
      artifacts: {
  ...
  enableJsonOutput(objective) {
    if (!this.jsonOutputAggregator) {
      this.jsonOutputAggregator = new import_json_output_aggregator.SwarmJsonOutputAggregator(
        this.swarmId.id,
        objective,
        this.config
      );
