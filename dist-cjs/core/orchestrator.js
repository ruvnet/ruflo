...
class SessionManager {
  constructor(terminalManager, memoryManager, eventBus, logger, config) {
    ...
    this.logger = logger;
    this.config = config;
    this.persistencePath = (0, import_path.join)(config.orchestrator.dataDir || "./data", "sessions.json");
    this.persistenceCircuitBreaker = (0, import_helpers.circuitBreaker)("SessionPersistence", {
      threshold: 5,
      timeout: 3e4,
      resetTimeout: 6e4
    });
  ...
  async createSession(profile) {
    try {
      const terminalId = await (0, import_helpers.retry)(() => this.terminalManager.spawnTerminal(profile), {
        maxAttempts: 3,
        initialDelay: 1e3
      });
      const memoryBankId = await (0, import_helpers.retry)(() => this.memoryManager.createBank(profile.id), {
        maxAttempts: 3,
        initialDelay: 1e3
      });
      const session = {
      ...
      this.persistSessions().catch(
        (error) => this.logger.error("Failed to persist sessions", error)
      );
      return session;
    } catch (error) {
      this.logger.error("Failed to create session", { agentId: profile.id, error });
      throw new import_errors.SystemError(`Failed to create session for agent ${profile.id}`, { error });
    }
  }
  ...
  async terminateSession(sessionId) {
    ...
    try {
      ...
      await Promise.race([
        ...
        (0, import_helpers.delay)(5e3).then(() => {
  ...
  async persistSessions() {
    ...
    try {
      await this.persistenceCircuitBreaker.execute(async () => {
        ...
        await (0, import_promises.mkdir)((0, import_path.dirname)(this.persistencePath), { recursive: true });
        await (0, import_promises.writeFile)(this.persistencePath, JSON.stringify(data, null, 2), "utf8");
  ...
  async restoreSessions() {
    ...
    try {
      const data = await (0, import_promises.readFile)(this.persistencePath, "utf8");
...
class Orchestrator {
  constructor(config, terminalManager, memoryManager, coordinationManager, mcpServer, eventBus, logger) {
    ...
    this.configManager = import_config_manager.ConfigManager.getInstance();
    this.healthCheckCircuitBreaker = (0, import_helpers.circuitBreaker)("HealthCheck", {
      threshold: 3,
      timeout: 1e4,
      resetTimeout: 3e4
    });
    this.taskAssignmentCircuitBreaker = (0, import_helpers.circuitBreaker)("TaskAssignment", {
      threshold: 5,
      timeout: 5e3,
      resetTimeout: 2e4
    });
  ...
  async initialize() {
    if (this.initialized) {
      throw new import_errors.InitializationError("Orchestrator already initialized");
    }
    ...
    try {
      ...
      if (this.configManager.isClaudeAPIConfigured()) {
        try {
          this.claudeClient = new import_claude_client.ClaudeAPIClient(this.logger, this.configManager);
      ...
      this.eventBus.emit(import_types.SystemEvents.SYSTEM_READY, { timestamp: /* @__PURE__ */ new Date() });
      this.logger.info("Orchestrator initialized successfully", { duration: initDuration });
    } catch (error) {
      this.logger.error("Failed to initialize orchestrator", error);
      await this.emergencyShutdown();
      throw new import_errors.InitializationError("Orchestrator", { error });
    }
  }
  async shutdown() {
    ...
    try {
      ...
      await Promise.race([
        ...
        (0, import_helpers.delay)(this.config.orchestrator.shutdownTimeout)
      ]);
      const shutdownDuration = Date.now() - shutdownStart;
      this.eventBus.emit(import_types.SystemEvents.SYSTEM_SHUTDOWN, { reason: "Graceful shutdown" });
      this.logger.info("Orchestrator shutdown complete", { duration: shutdownDuration });
    } catch (error) {
      this.logger.error("Error during shutdown", error);
      await this.emergencyShutdown();
      throw new import_errors.ShutdownError("Failed to shutdown gracefully", { error });
    } finally {
  ...
  async spawnAgent(profile) {
    if (!this.initialized) {
      throw new import_errors.SystemError("Orchestrator not initialized");
    }
    if (this.agents.size >= this.config.orchestrator.maxConcurrentAgents) {
      throw new import_errors.SystemError("Maximum concurrent agents reached");
    }
    this.validateAgentProfile(profile);
    this.logger.info("Spawning agent", { agentId: profile.id, type: profile.type });
    try {
      const session = await (0, import_helpers.retry)(() => this.sessionManager.createSession(profile), {
        maxAttempts: 3,
        initialDelay: 2e3
      });
      this.agents.set(profile.id, profile);
      this.eventBus.emit(import_types.SystemEvents.AGENT_SPAWNED, {
        agentId: profile.id,
        profile,
        sessionId: session.id
      });
  ...
  async terminateAgent(agentId) {
    ...
    if (!profile) {
      throw new import_errors.SystemError(`Agent not found: ${agentId}`);
    }
    ...
    try {
      ...
      this.eventBus.emit(import_types.SystemEvents.AGENT_TERMINATED, {
        agentId,
      ...
      await this.taskAssignmentCircuitBreaker.execute(async () => {
        if (!task.assignedAgent) {
          if (this.taskQueue.length >= this.config.orchestrator.taskQueueSize) {
            throw new import_errors.SystemError("Task queue is full");
          }
          this.taskQueue.push(task);
          this.eventBus.emit(import_types.SystemEvents.TASK_CREATED, { task });
          await this.processTaskQueue();
          return;
        }
        const agent = this.agents.get(task.assignedAgent);
        if (!agent) {
          throw new import_errors.SystemError(`Agent not found: ${task.assignedAgent}`);
        }
        await this.coordinationManager.assignTask(task, task.assignedAgent);
        this.eventBus.emit(import_types.SystemEvents.TASK_ASSIGNED, {
          taskId: task.id,
  ...
  startHealthChecks() {
    this.healthCheckInterval = setInterval(async () => {
      try {
        ...
        this.eventBus.emit(import_types.SystemEvents.SYSTEM_HEALTHCHECK, { status: health });
  ...
  startMaintenanceTasks() {
    ...
    }, this.config.orchestrator.maintenanceInterval || 3e5);
  }
  startMetricsCollection() {
    ...
    }, this.config.orchestrator.metricsInterval || 6e4);
  }
  stopBackgroundTasks() {
  ...
  async getComponentHealth(name, check) {
    try {
      const result = await Promise.race([
        check(),
        (0, import_helpers.delay)(5e3).then(() => ({ healthy: false, error: "Health check timeout" }))
      ]);
      const health = {
        name,
      ...
      if ("metrics" in result && result.metrics !== void 0) {
  ...
  async initializeComponent(name, init) {
    try {
      await (0, import_helpers.retry)(init, { maxAttempts: 3, initialDelay: 2e3 });
      this.logger.info(`${name} initialized`);
    } catch (error) {
      this.logger.error(`Failed to initialize ${name}`, error);
      throw new import_errors.InitializationError(name, { error });
    }
  }
  async shutdownComponent(name, shutdown) {
    try {
      await Promise.race([
        shutdown(),
        (0, import_helpers.delay)(1e4)
        // 10 second timeout per component
      ]);
  ...
  async handleAgentError(agentId, error) {
    ...
    if (errorCount < 3) {
      try {
        ...
        await (0, import_helpers.delay)(2e3);
    ...
    try {
      ...
      for (const task of tasks) {
        ...
        if (trackedTask) {
          ...
          trackedTask.completedAt = /* @__PURE__ */ new Date();
        }
        this.eventBus.emit(import_types.SystemEvents.TASK_CANCELLED, {
          taskId: task.id,
  ...
  async cleanupTerminatedSessions() {
    ...
    const cutoffTime = Date.now() - (this.config.orchestrator.sessionRetentionMs || 36e5);
  ...
  async cleanupTaskHistory() {
    const cutoffTime = Date.now() - (this.config.orchestrator.taskHistoryRetentionMs || 864e5);
