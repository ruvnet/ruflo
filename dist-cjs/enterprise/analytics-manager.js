...
var analytics_manager_exports = {};
__export(analytics_manager_exports, {
  AnalyticsManager: () => AnalyticsManager
});
module.exports = __toCommonJS(analytics_manager_exports);
var import_events = require("events");
var import_promises = require("fs/promises");
var import_path = require("path");
var import_logger = require("../core/logger.js");
var import_config = require("../core/config.js");
class AnalyticsManager extends import_events.EventEmitter {
  static {
  ...
  async initialize() {
    try {
      await (0, import_promises.mkdir)(this.analyticsPath, { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.analyticsPath, "metrics"), { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.analyticsPath, "dashboards"), { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.analyticsPath, "insights"), { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.analyticsPath, "models"), { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.analyticsPath, "reports"), { recursive: true });
  ...
  async recordMetric(metric) {
    ...
    const retentionPeriod = 24 * 60 * 60 * 1e3;
  ...
  async makePrediction(modelId, input) {
    ...
    model.lastPrediction = /* @__PURE__ */ new Date();
    await this.saveModel(model);
  ...
  async getPerformanceMetrics(timeRange) {
    const range = timeRange || {
      start: new Date(Date.now() - 60 * 60 * 1e3),
      // Last hour
      end: /* @__PURE__ */ new Date()
    };
    ...
    return {
      system: {
        cpu: {
          ...
          cores: 8,
          // Would be detected from system
          loadAverage: [1.2, 1.5, 1.8]
          // Would be collected from system
        },
        memory: {
  ...
  async getUsageMetrics(timeRange) {
    ...
    return {
      ...
      features: {
        adoption: {
          dashboard: { users: 800, usage: 5e3, retention: 85 },
          reports: { users: 600, usage: 2e3, retention: 70 },
  ...
  async loadConfigurations() {
    try {
      const dashboardFiles = await (0, import_promises.readdir)((0, import_path.join)(this.analyticsPath, "dashboards"));
      for (const file of dashboardFiles.filter((f) => f.endsWith(".json"))) {
        const content = await (0, import_promises.readFile)((0, import_path.join)(this.analyticsPath, "dashboards", file), "utf-8");
        const dashboard = JSON.parse(content);
        this.dashboards.set(dashboard.id, dashboard);
      }
      const insightFiles = await (0, import_promises.readdir)((0, import_path.join)(this.analyticsPath, "insights"));
      for (const file of insightFiles.filter((f) => f.endsWith(".json"))) {
        const content = await (0, import_promises.readFile)((0, import_path.join)(this.analyticsPath, "insights", file), "utf-8");
        const insight = JSON.parse(content);
        this.insights.set(insight.id, insight);
      }
      const modelFiles = await (0, import_promises.readdir)((0, import_path.join)(this.analyticsPath, "models"));
      for (const file of modelFiles.filter((f) => f.endsWith(".json"))) {
        const content = await (0, import_promises.readFile)((0, import_path.join)(this.analyticsPath, "models", file), "utf-8");
  ...
  async persistMetric(metric) {
    ...
    const filePath = (0, import_path.join)(this.analyticsPath, "metrics", `${date}.json`);
    try {
      let existingData = [];
      try {
      ...
      await (0, import_promises.writeFile)(filePath, JSON.stringify(existingData, null, 2));
    } catch (error) {
  ...
  aggregateMetrics(metrics, aggregation) {
    ...
    const buckets = {};
    for (const metric of metrics) {
      const bucket = new Date(Math.floor(metric.timestamp.getTime() / 6e4) * 6e4).toISOString();
      if (!buckets[bucket]) {
        buckets[bucket] = [];
      }
  ...
  async saveDashboard(dashboard) {
    const filePath = (0, import_path.join)(this.analyticsPath, "dashboards", `${dashboard.id}.json`);
    await (0, import_promises.writeFile)(filePath, JSON.stringify(dashboard, null, 2));
  }
  async saveInsight(insight) {
    const filePath = (0, import_path.join)(this.analyticsPath, "insights", `${insight.id}.json`);
    await (0, import_promises.writeFile)(filePath, JSON.stringify(insight, null, 2));
  }
  async saveModel(model) {
    const filePath = (0, import_path.join)(this.analyticsPath, "models", `${model.id}.json`);
    await (0, import_promises.writeFile)(filePath, JSON.stringify(model, null, 2));
  }
}
