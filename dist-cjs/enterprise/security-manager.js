...
var security_manager_exports = {};
__export(security_manager_exports, {
  SecurityManager: () => SecurityManager
});
module.exports = __toCommonJS(security_manager_exports);
...
class SecurityManager extends import_events.EventEmitter {
  static {
  ...
  vulnerabilityDatabases = /* @__PURE__ */ new Map();
  securityPath;
  logger;
  config;
  ...
  async initialize() {
    try {
      await (0, import_promises.mkdir)(this.securityPath, { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.securityPath, "scans"), { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.securityPath, "policies"), { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.securityPath, "incidents"), { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.securityPath, "reports"), { recursive: true });
      await (0, import_promises.mkdir)((0, import_path.join)(this.securityPath, "databases"), { recursive: true });
  ...
  async createSecurityScan(scanData) {
    const scan = {
      ...
      configuration: {
        ...
        formats: ["json", "html"],
        outputPath: (0, import_path.join)(this.securityPath, "reports"),
  ...
  async executeScan(scanId) {
    ...
    scan.status = "running";
    scan.updatedAt = /* @__PURE__ */ new Date();
  ...
  async createSecurityIncident(incidentData) {
    const incident = {
      ...
      timeline: {
        ...
        reported: /* @__PURE__ */ new Date(),
        acknowledged: /* @__PURE__ */ new Date()
      },
      response: {
        assignedTo: [],
        actions: [],
  ...
  async loadConfigurations() {
    try {
      const scanFiles = await (0, import_promises.readdir)((0, import_path.join)(this.securityPath, "scans"));
      for (const file of scanFiles.filter((f) => f.endsWith(".json"))) {
        const content = await (0, import_promises.readFile)((0, import_path.join)(this.securityPath, "scans", file), "utf-8");
        const scan = JSON.parse(content);
        this.scans.set(scan.id, scan);
      }
      const policyFiles = await (0, import_promises.readdir)((0, import_path.join)(this.securityPath, "policies"));
      for (const file of policyFiles.filter((f) => f.endsWith(".json"))) {
        const content = await (0, import_promises.readFile)((0, import_path.join)(this.securityPath, "policies", file), "utf-8");
        const policy = JSON.parse(content);
        this.policies.set(policy.id, policy);
      }
      const incidentFiles = await (0, import_promises.readdir)((0, import_path.join)(this.securityPath, "incidents"));
      for (const file of incidentFiles.filter((f) => f.endsWith(".json"))) {
        const content = await (0, import_promises.readFile)((0, import_path.join)(this.securityPath, "incidents", file), "utf-8");
  ...
  async saveScan(scan) {
    const filePath = (0, import_path.join)(this.securityPath, "scans", `${scan.id}.json`);
    await (0, import_promises.writeFile)(filePath, JSON.stringify(scan, null, 2));
  }
  async savePolicy(policy) {
    const filePath = (0, import_path.join)(this.securityPath, "policies", `${policy.id}.json`);
    await (0, import_promises.writeFile)(filePath, JSON.stringify(policy, null, 2));
  }
  async saveIncident(incident) {
    const filePath = (0, import_path.join)(this.securityPath, "incidents", `${incident.id}.json`);
    await (0, import_promises.writeFile)(filePath, JSON.stringify(incident, null, 2));
  }
