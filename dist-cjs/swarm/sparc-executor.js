...
var sparc_executor_exports = {};
__export(sparc_executor_exports, {
  SparcTaskExecutor: () => SparcTaskExecutor
});
module.exports = __toCommonJS(sparc_executor_exports);
var fs = __toESM(require("node:fs/promises"), 1);
var path = __toESM(require("node:path"), 1);
var import_logger = require("../core/logger.js");
class SparcTaskExecutor {
  static {
    __name(this, "SparcTaskExecutor");
  }
  logger;
  enableTDD;
  qualityThreshold;
  ...
  determineAppType(description) {
    ...
    if (desc.includes("rest api") || desc.includes("api"))
      return "rest-api";
    if (desc.includes("flask") || desc.includes("fastapi"))
      return "python-web";
  ...
  detectLanguage(description) {
    ...
    if (desc.includes("python") || desc.includes("flask") || desc.includes("pandas"))
      return "python";
    if (desc.includes("typescript") || desc.includes("ts"))
      return "typescript";
  ...
  async writeImplementationFiles(targetDir, implementation, language) {
    ...
    for (const [module2, content] of Object.entries(implementation.modules)) {
      const filename = this.getSourceFileName(module2, language);
...
//# sourceMappingURL=sparc-executor.js.map
