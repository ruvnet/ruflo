...
**Note (2026-09-19):** 2026-09-15 (performance, #3329/#3330), 2026-09-16 (security, #3338/#3339), 2026-09-17 (intelligence, #3349/#3350), and 2026-09-18 (memory, #3351/#3352) all exist as real branches+issues+draft-PRs (verified via `mcp__github__search_pull_requests`, not inferred) but are not transcribed as rows here — same "ledger rows only reach `main` via PR merge" structural gap noted repeatedly since 2026-08-19/09-03/09-05/09-09. GitHub-verified fates for the trailing 7 nights before tonight's own row (via `mcp__github__pull_request_read`, `gh` CLI unavailable this session): #3302 (09-12) **MERGED**; #3291 (09-11) **MERGED**; #3266 (09-10) **MERGED**; #3330 (09-15) OPEN/draft 4d old; #3339 (09-16) OPEN/draft 3d old; #3350 (09-17) OPEN/draft 2d old; #3352 (09-18) OPEN/draft 1d old — none STALE. A large batch (18 dream PRs spanning 08-14..09-12) merged together on 2026-09-14. Swarm surface specifically: #3086 (08-24), #3123 (08-29), #3243 (09-09) — 3/3 merged, 100% historical rate.
| 2026-09-19 | swarm | `TopologyManager.createEdgesForNode()` mirrored a new node's edge onto its connection target only for `type === 'mesh'` — for hierarchical/centralized/hybrid, the queen/coordinator's own `connections`/`adjacencyList` permanently never gained any worker (not just transiently stale: neither `shouldRebalance()` nor any `rebalance*()` guard, which checks the worker's side instead of the leader's, could ever repair it); fixed by mirroring whenever the connection target already exists in the topology | #3356 | #3357 | yes | ACCEPT-scoped | baseline 3/3 new tests fail (`expected [] to include`), candidate 233/233 full-suite pass (was 230/230 pre-diff), `tsc` clean; independent adversarial critique CONFIRMED — disclosed caveat: no shipped caller of `getNeighbors()`/`findOptimalPath()` exists yet and `swarm-tools.ts`'s `getStatus()` has a separate pre-existing type-drift bug, so the repaired state is correct/live but not yet externally visible | 2cd2ea40... | 08-24..09-12 merge status above; 09-15..09-18 still open/draft, none stale |
