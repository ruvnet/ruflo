...
case "${1:-status}" in
    ...
    "help"|"-h"|"--help")
        ...
        echo ""
        echo "Commands:"
        echo "  start [swarm] [metrics] [profile]          Start all daemons"
        echo "  stop                                       Stop all daemons"
        echo "  restart [swarm] [metrics] [profile]        Restart all daemons"
        echo "  status                                     Show daemon status"
        echo "  start-swarm [interval]                     Start swarm monitor only"
        echo "  start-metrics [interval]                   Start metrics daemon only"
        echo "  start-cache [profile]                      Start cache optimizer (GNN/GRNN)"
        echo "  cache-status                               Show GNN/GRNN status"
        echo "  cache-report [format]                      Generate GNN/GRNN report"
        echo "  help                                       Show this help"
        echo ""
        echo "Cache Profiles: single-agent, multi-agent, aggressive, conservative"
        echo ""
        echo "Examples:"
