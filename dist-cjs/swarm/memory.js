...
class SwarmMemoryManager extends import_node_events.EventEmitter {
  ...
  async shareMemory(key, targetAgent, options = {}) {
    ...
    const sharedEntryId = (0, import_helpers.generateId)("shared-mem");
    const sharedEntry = {
      ...entry,
      id: sharedEntryId,
      owner: targetAgent,
  ...
  async createPartition(name, options = {}, skipInitCheck = false) {
    ...
    const partition = {
      id: (0, import_helpers.generateId)("partition"),
      name,
      type: options.type || "knowledge",
      entries: [],
  ...
  async createBackup() {
    ...
    const backupId = (0, import_helpers.generateId)("backup");
  ...
  getStatistics() {
    ...
    let totalSize = 0;
    let oldestEntry = /* @__PURE__ */ new Date();
    let newestEntry = /* @__PURE__ */ new Date(0);
    ...
    for (const entry of validEntries) {
      ...
      if (entry.expiresAt && entry.expiresAt.getTime() - Date.now() < 24 * 60 * 60 * 1e3) {
        expiringEntries++;
      }
    }
    return {
  ...
  isExpired(entry) {
    return entry.expiresAt ? entry.expiresAt <= /* @__PURE__ */ new Date() : false;
  }
  ...
  indexTerms(entryId, terms) {
    for (const term of terms) {
      ...
      if (!this.index.has(normalizedTerm)) {
        this.index.set(normalizedTerm, /* @__PURE__ */ new Set());
      }
...
class MemoryReplication extends import_node_events.EventEmitter {
  static {
    __name(this, "MemoryReplication");
  }
  config;
  constructor(config) {
    super();
...
var memory_default = SwarmMemoryManager;
