...
describe('Performance Tests', () => {
      ...
      const { duration } = await perfHelpers.measureTime(async () => {
        ...
        for (let i = 0; i < 20; i++) {
          operations.push((async () => {
            try {
              await memoryCommand(['store', `key${i}`, `value${i}`], {});
            } catch (error) {
              // Expected to fail in test environment
            }
          })());
  ...
  describe('Benchmarks', () => {
    test('should meet performance benchmarks for CLI startup', async () => {
      ...
      const { result, duration } = await perfHelpers.measureTime(mockStartupOperations);
    ...
    test('should meet performance benchmarks for swarm initialization', async () => {
      ...
      const { result, duration } = await perfHelpers.measureTime(mockSwarmInit);
