...
Complete reference for all Gemini-Flow command-line interface commands, options, and usage patterns.
...
```bash
gemini-flow [global-options] <command> [command-options]
```
...
|--------|-------|-------------|---------|
| `--config <path>` | `-c` | Path to configuration file | `./gemini-flow.config.json` |
...
gemini-flow --config ./my-config.json --verbose agent list

# JSON output with debug logging
gemini-flow --json --log-level debug task list

# Quiet mode for scripts
gemini-flow --quiet --no-color start --daemon
```
...
gemini-flow start --daemon --port 3001

# Start with HTTP MCP transport
gemini-flow start --mcp-transport http --port 8000

# Start with custom worker count
gemini-flow start --workers 4 --memory-limit 1024
```
...
```bash
gemini-flow agent <subcommand> [options]
```
...
gemini-flow agent spawn researcher --name "Research Bot"
...
gemini-flow agent spawn custom \
  --name "Special Agent" \
...
gemini-flow agent list --status active --type researcher

# JSON output sorted by creation time
gemini-flow agent list --format json --sort created
```
...
```bash
gemini-flow agent info agent_1704123456789_researcher --detailed --stats
```
...
```bash
gemini-flow agent terminate agent_123 --reason "Task completed"
```

#### `update` - Update Agent

```bash
gemini-flow agent update <agent-id> [options]
```
...
```bash
gemini-flow task <subcommand> [options]
```
...
gemini-flow task create research "Research quantum computing trends"

# High-priority analysis with deadline
gemini-flow task create analysis "Analyze user behavior data" \
  --priority high \
  --deadline "2024-01-15T17:00:00Z" \
  --assign-to agent_123

# Implementation task with dependencies
gemini-flow task create implementation "Implement user authentication" \
...
```bash
gemini-flow task status <task-id> [options]
```
...
```bash
gemini-flow task cancel <task-id> [options]
```
...
```bash
gemini-flow task retry <task-id> [options]
```
...
```bash
gemini-flow memory <subcommand> [options]
```

#### `query` - Query Memory

```bash
gemini-flow memory query [options]
```
...
```bash
# Query by category
gemini-flow memory query --category research --limit 10

# Semantic search
gemini-flow memory query --vector-search "machine learning algorithms"
...
```bash
gemini-flow memory stats [options]
```
...
```bash
gemini-flow memory cleanup [options]
```
...
```bash
gemini-flow config <subcommand> [options]
```

#### `init` - Initialize Configuration

```bash
gemini-flow config init [file] [options]
```
...
```bash
gemini-flow config show [options]
```
...
```bash
gemini-flow config get <path> [options]
```

**Examples:**

```bash
gemini-flow config get orchestrator.maxConcurrentAgents
gemini-flow config get memory.backend
```

#### `set` - Set Config Value

```bash
gemini-flow config set <path> <value> [options]
```

**Examples:**

```bash
gemini-flow config set orchestrator.maxConcurrentAgents 10
gemini-flow config set memory.cacheSizeMB 200
```

#### `validate` - Validate Configuration

```bash
gemini-flow config validate [file] [options]
```
...
```bash
gemini-flow workflow <subcommand> [options]
```

#### `execute` - Execute Workflow

```bash
gemini-flow workflow execute <file> [options]
```
...
```bash
gemini-flow workflow status <workflow-id> [options]
```

#### `list` - List Workflows

```bash
gemini-flow workflow list [options]
```

#### `template` - Workflow Templates

```bash
gemini-flow workflow template <subcommand>
```
...
gemini-flow monitor --interval 5

# Compact mode for smaller screens
gemini-flow monitor --compact

# Focus on specific component
gemini-flow monitor --focus orchestrator
```

---

### `claude` - Spawn Gemini Instances

Spawn and manage Gemini Code instances with specific configurations, similar to claude-sparc.sh but integrated into the orchestration system.

```bash
gemini-flow gemini <subcommand> [options]
```

#### `spawn` - Spawn Single Gemini Instance

```bash
gemini-flow gemini spawn <task> [options]
```
...
```bash
# Basic task
gemini-flow gemini spawn "implement user authentication"

# Research task
gemini-flow gemini spawn "research microservices patterns" --research --parallel

# Backend development
gemini-flow gemini spawn "create REST API" --mode backend-only --coverage 90

# Frontend with no permissions
gemini-flow gemini spawn "build dashboard" --mode frontend-only --no-permissions

# Dry run
gemini-flow gemini spawn "refactor code" --dry-run
```

#### `batch` - Execute Workflow File

```bash
gemini-flow gemini batch <workflow-file> [options]
```

Execute multiple Gemini instances from a JSON workflow file.
...
```bash
# Execute workflow
gemini-flow gemini batch workflow.json

# Dry run
gemini-flow gemini batch workflow.json --dry-run
```
...
```bash
gemini-flow session <subcommand> [options]
```

#### `list` - List Sessions

```bash
gemini-flow session list
```

#### `attach` - Attach to Session

```bash
gemini-flow session attach <session-id>
```

#### `terminate` - Terminate Session

```bash
gemini-flow session terminate <session-id>
```
...
In REPL mode, you can use any command without the `gemini-flow` prefix:

```
gemini-flow> agent list
gemini-flow> task create research "AI trends"
gemini-flow> memory query --category research
gemini-flow> help agent
gemini-flow> exit
```

## Environment Variables

Gemini-Flow recognizes these environment variables:

| Variable | Description | Default |
|----------|-------------|---------|
| `CLAUDE_FLOW_CONFIG` | Default configuration file path | `./gemini-flow.config.json` |
...
```bash
# Bash
gemini-flow completion bash > /usr/local/etc/bash_completion.d/gemini-flow

# Zsh
gemini-flow completion zsh > /usr/local/share/zsh/site-functions/_gemini-flow

# Fish
gemini-flow completion fish > ~/.config/fish/completions/gemini-flow.fish

# PowerShell
gemini-flow completion powershell > $PROFILE
```

Or install automatically:

```bash
gemini-flow completion --install
```
...
alias cfs='gemini-flow status'
alias cfa='gemini-flow agent'
alias cft='gemini-flow task'
alias cfm='gemini-flow memory'
```
...
gemini-flow --quiet --json agent list > agents.json

# Check exit code
if gemini-flow --quiet task status $TASK_ID; then
...
```bash
# Development
gemini-flow --config dev.config.json start

# Production
gemini-flow --config prod.config.json start --daemon

# Testing
gemini-flow --config test.config.json task create test "Run tests"
```
...
This completes the comprehensive CLI reference for Gemini-Flow. Each command includes detailed options, examples, and best practices for effective usage.