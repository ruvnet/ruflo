...
export class IntegrationRegressionSuite {
  ...
  private registerDefaultTests(): void {
    ...
    this.tests.push({
      ...
      timeout: 5000,
      run: async () => {
        const { EventBus, createAgentSpawnedEvent } = await import('@claude-flow/core');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const { HookRegistry, HookPriority } = await import('@claude-flow/core');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          ...
          const shared = await import('@claude-flow/core');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const swarm = await import('@claude-flow/agents/swarm');
