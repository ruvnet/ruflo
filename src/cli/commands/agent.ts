...
export function createAgentCommand(): Command {
  const agentCommand = new Command('agent')
    .description('Comprehensive Gemini-Flow agent management with advanced features')
    .action(() => {
      console.log(chalk.cyan('🤖 Gemini-Flow Agent Management System'));
      console.log('');
    ...
    .action(async (template: string, options: any) => {
      try {
        ...
        if (options.start) {
          ...
          console.log(chalk.yellow(`Use 'gemini-flow agent start ${agentId}' to start the agent`));
        }
