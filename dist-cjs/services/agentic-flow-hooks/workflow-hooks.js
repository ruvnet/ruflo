...
var workflow_hooks_exports = {};
__export(workflow_hooks_exports, {
  registerWorkflowHooks: () => registerWorkflowHooks,
  workflowCompleteHook: () => workflowCompleteHook,
  workflowDecisionHook: () => workflowDecisionHook,
  workflowErrorHook: () => workflowErrorHook,
  workflowStartHook: () => workflowStartHook,
  workflowStepHook: () => workflowStepHook
});
module.exports = __toCommonJS(workflow_hooks_exports);
var import_hook_manager = require("./hook-manager.js");
const workflowStartHook = {
...
__name(loadWorkflowHistory, "loadWorkflowHistory");
...
__name(loadWorkflowLearnings, "loadWorkflowLearnings");
...
__name(selectOptimalProvider, "selectOptimalProvider");
...
__name(generateWorkflowPredictions, "generateWorkflowPredictions");
...
__name(getStepOptimizations, "getStepOptimizations");
...
__name(applyStepOptimizations, "applyStepOptimizations");
...
__name(summarizeState, "summarizeState");
...
__name(getDecisionOutcomes, "getDecisionOutcomes");
function adjustDecisionConfidence(decision, historicalOutcomes) {
  if (historicalOutcomes.length === 0) {
    return decision;
  }
  const relevantOutcomes = historicalOutcomes.filter(
    (o) => o.selected === decision.selected
  );
...
__name(adjustDecisionConfidence, "adjustDecisionConfidence");
...
__name(generateAlternativeDecisions, "generateAlternativeDecisions");
...
__name(calculateWorkflowPerformance, "calculateWorkflowPerformance");
...
async function generateImprovementSuggestions(workflowId, state, performance, learnings, context) {
  ...
  const providerLearnings = learnings.filter(
    (l) => l.type === "success" && l.value.provider
  );
...
__name(generateImprovementSuggestions, "generateImprovementSuggestions");
...
__name(analyzeErrorPattern, "analyzeErrorPattern");
...
__name(findRecoveryStrategy, "findRecoveryStrategy");
...
__name(applyRecoveryStrategy, "applyRecoveryStrategy");
...
__name(selectAlternativeProvider, "selectAlternativeProvider");
...
__name(calculateAlternativeConfidence, "calculateAlternativeConfidence");
function extractDecisions(state) {
  return state.decisions || [];
}
__name(extractDecisions, "extractDecisions");
...
function registerWorkflowHooks() {
  import_hook_manager.agenticHookManager.register(workflowStartHook);
  import_hook_manager.agenticHookManager.register(workflowStepHook);
  import_hook_manager.agenticHookManager.register(workflowDecisionHook);
  import_hook_manager.agenticHookManager.register(workflowCompleteHook);
  import_hook_manager.agenticHookManager.register(workflowErrorHook);
}
__name(registerWorkflowHooks, "registerWorkflowHooks");
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  registerWorkflowHooks,
  workflowCompleteHook,
  workflowDecisionHook,
  workflowErrorHook,
  workflowStartHook,
  workflowStepHook
});
