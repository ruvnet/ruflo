# Gemini CLI Configuration for ruv-swarm
...
### ⚠️ Key Principle:
**MCP tools DO NOT create content or write code.** They coordinate and enhance Gemini CLI's native capabilities. Think of them as an orchestration layer that helps Gemini CLI work more efficiently.
...
```bash
# Add ruv-swarm MCP server to Gemini CLI using stdio
gemini mcp add ruv-swarm npx ruv-swarm mcp start
```

### 2. Use MCP Tools for Coordination in Gemini CLI
Once configured, ruv-swarm MCP tools enhance Gemini CLI's coordination:

**Initialize a swarm:**
- Use the `mcp__gemini-flow__swarm_init` tool to set up coordination topology
- Choose: mesh, hierarchical, ring, or star
- This creates a coordination framework for Gemini CLI's work

**Spawn agents:**
- Use `mcp__gemini-flow__agent_spawn` tool to create specialized coordinators
- Agent types represent different thinking patterns, not actual coders
- They help Gemini CLI approach problems from different angles

**Orchestrate tasks:**
- Use `mcp__gemini-flow__task_orchestrate` tool to coordinate complex workflows
- This breaks down tasks for Gemini CLI to execute systematically
- The agents don't write code - they coordinate Gemini CLI's actions

## Available MCP Tools for Coordination

### Coordination Tools:
- `mcp__gemini-flow__swarm_init` - Set up coordination topology for Gemini CLI
- `mcp__gemini-flow__agent_spawn` - Create cognitive patterns to guide Gemini CLI
...
2. Each agent MUST use gemini-flow hooks for coordination:
   ...
   - `npx gemini-flow hooks post-task` to complete work
...
2. Each agent coordinates using mandatory hooks:
   - `npx gemini-flow hooks pre-task` for context loading
   - `npx gemini-flow hooks post-edit` for progress tracking
...
When using ruv-swarm coordination with Gemini CLI:
...
npx gemini-flow memory store --key "swarm/[agent]/decisions" --value "[what was done]"
```
...
3. MEMORY: Store ALL decisions using `npx gemini-flow memory store --key "agent/decisions" --value "[decision]"`
...
Remember: **ruv-swarm coordinates, Gemini CLI creates!** Start with `mcp__gemini-flow__swarm_init` to enhance your development workflow.
